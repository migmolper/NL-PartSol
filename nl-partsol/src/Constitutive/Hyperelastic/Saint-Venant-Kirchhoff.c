  ...
  */
  ...
  for (int i = 0; i < Ndim; i++) {
    for (int j = 0; j < Ndim; j++) {
      P.N[i][j] = lambda * 0.5 * (tr__Ft_x_F - Ndim) * F.N[i][j] +
                  G * (F_x_Ft_x_F.N[i][j] - F.N[i][j]);
    }
  }
...
Tensor grad_energy_Saint_Venant_Kirchhoff(Tensor grad_e, Tensor C,
                                          Material MatProp_p) {
  /* Number of dimensions */
  int Ndim = NumberDimensions;
...
Tensor compute_stiffness_density_Saint_Venant_Kirchhoff(Tensor v, Tensor w,
                                                        Material MatProp) {
  ...
  */
  ...
  for (int A = 0; A < Ndim; A++) {
    for (int B = 0; B < Ndim; B++) {
      C_mat.N[A][B] += lambda * v_o_w.N[A][B] + G * v_o_w.N[B][A] +
                       (A == B ? 1 : 0) * G * v_dot_w;
    }
  }
