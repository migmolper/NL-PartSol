...
State_Parameters compute_1PK_Stress_Tensor_Newtonian_Fluid_Incompressible(
    State_Parameters Input_SP, Material MatProp_p) {
  /*
  ...
  */
  ...
  for (int i = 0; i < Ndim; i++) {
    for (int j = 0; j < Ndim; j++) {
      P.N[i][j] = -Pressure * FmT.N[i][j] + 2 * J * mu * d__x__FmT.N[i][j] -
...
Tensor compute_stiffness_density_Newtonian_Fluid_Incompressible(
    Tensor GRAD_pA, Tensor GRAD_pB, Tensor F, Tensor dFdt, double J,
    double alpha4, Material MatProp_p) {
  ...
  */
  ...
  Tensor Fm1GRAD_o_FmTGRAD_pAB =
      dyadic_Product__TensorLib__(FmTGRAD_pA, FmTGRAD_pB);
  Tensor Fm1GRAD_o_FmTGRAD_pBA =
      dyadic_Product__TensorLib__(FmTGRAD_pB, FmTGRAD_pA);

  Tensor d_Fm1GRAD_o_FmTGRAD_pAB =
      matrix_product_old__TensorLib__(d, Fm1GRAD_o_FmTGRAD_pAB);
  Tensor d_Fm1GRAD_o_FmTGRAD_pBA =
      matrix_product_old__TensorLib__(d, Fm1GRAD_o_FmTGRAD_pBA);

  Tensor Fm1GRAD_o_FmTGRAD_pAB_dFdt_Fm1 =
      matrix_product_old__TensorLib__(Fm1GRAD_o_FmTGRAD_pAB, dFdt_Fm1);
  Tensor Fm1GRAD_o_FmTGRAD_pBA_dFdt_Fm1 =
      matrix_product_old__TensorLib__(Fm1GRAD_o_FmTGRAD_pBA, dFdt_Fm1);
  ...
  for (int i = 0; i < Ndim; i++) {
    ...
    for (int j = 0; j < Ndim; j++) {
      A.N[i][j] += -((2.0 / ((double)Ndim)) * alpha4 * J * mu) *
                       Fm1GRAD_o_FmTGRAD_pAB.N[i][j] +
                   ...
                   (2.0 / ((double)Ndim)) * J * mu *
                       Fm1GRAD_o_FmTGRAD_pAB_dFdt_Fm1.N[i][j];
    }
  }
