...
Tensor compute_stiffness_density_Newtonian_Fluid(Tensor GRAD_I, Tensor GRAD_J,
                                                 Tensor F, Tensor dFdt,
                                                 double J, double alpha4,
                                                 Material MatProp_p) {
  ...
  */
  ...
  for (int i = 0; i < Ndim; i++) {
    for (int j = 0; j < Ndim; j++) {
      A.N[i][j] = +(-J * p0 - J * (K / n) * (pow(J, -n) - 1) +
                    K * pow(J, 1 - n) - (2.0 / 3.0) * alpha4 * J * mu) *
