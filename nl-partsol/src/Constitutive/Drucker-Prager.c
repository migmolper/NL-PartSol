...
static double compute_derivative_yield_surface_classical(double, double, double, Material);
...
static void   update_pq_classical(Tensor, Tensor, Tensor, double, double, Material);
...
static void   update_pq_apex(Tensor, Tensor, double, double, Material);
static void   update_Kirchhoff_stress_tensor_pq(Tensor, Tensor, Tensor, Material);
...
*/
{
  ...
  p = volumetric_component__TensorLib__(T_Kirchhoff_trial);
  q = deviatoric_component__TensorLib__(T_Kirchhoff_trial,p);
...
Plastic_status infinitesimal_strains_plasticity_Drucker_Prager_Sanavia(Tensor p, Tensor q,
                                                                       Plastic_status Inputs_VarCons,
                                                                       Material MatProp)
/*	
...
*/
{
  ...
  */ 
  ...
  p_norm = EuclideanNorm__TensorLib__(p);

  /*
    Yield condition : Starting from incremental plastic strain equal to zero
  */
  delta_Gamma = 0.0;
  Phi = compute_yield_surface(p_norm, q_norm, cohesion_k, MatProp);
  ...
  if(Phi > TOL)
  {
    ...
    p_lim = compute_limit_between_classic_apex_algorithm(q_norm, cohesion_k, H, K, G, MatProp);
    ...
    */
    if(p_norm <= p_lim)
    {
     ...
     while(Convergence == false)
     {
       ...
       d_Phi = compute_derivative_yield_surface_classical(H, K, G, MatProp);
      ...
      */
	  ...
	  /*
	    update
	  */
    D_plastic = compute_increment_plastic_strain_classical(q, q_norm, delta_Gamma, MatProp);
    
    update_pq_classical(p, q, q_norm, delta_Gamma, MatProp);
  ...
  */
  else
  {
    ...
    delta_Gamma_2 = 0.0;
    delta_Gamma = delta_Gamma1 + delta_Gamma2;

    while(Convergence == false)
    {

      d_Phi = compute_derivative_yield_surface_apex(H, q_norm, delta_Gamma, MatProp);

      delta_Gamma_2 = update_increment_plastic_strain(delta_Gamma_2, Phi, d_Phi);

      delta_Gamma = delta_Gamma1 + delta_Gamma2;

      Phi = compute_yield_surface_apex(p_norm, q_norm, H, delta_Gamma, cohesion_k, MatProp);
	...
	/*
	 update
	*/
  EPS_k = update_equivalent_plastic_strain_apex(EPS_k, q_norm, delta_Gamma, MatProp);

  D_plastic = compute_increment_plastic_strain_apex(q, q_norm, delta_Gamma,MatProp);

  update_pq_apex(p, q, q_norm, delta_Gamma, MatProp);
  ...
  */
  ...
  Outputs_VarCons.D_plastic = D_plastic;
  ...
  double G)
{
  ...
  for(int i = 0 ; i<Ndim ; i++)
  {
    p.N[i][i] = p.N[i][i] - 3*K*alpha_Q*delta_Gamma;
...
static void update_Kirchhoff_stress_tensor_pq(
  Tensor T,
  Tensor p,
  Tensor q)
{
