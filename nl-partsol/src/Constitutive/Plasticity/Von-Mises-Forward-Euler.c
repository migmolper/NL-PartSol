...
State_Parameters Von_Mises_forward_euler(State_Parameters Inputs_SP,
                                         Material MatProp)
/*
  ...
  if (Phi_tr > TOL) {
    ...
    *Inputs_SP.Equiv_Plast_Str = update_equivalent_plastic_strain(
    ...
    apply_plastic_corrector_stress_tensor(
        Inputs_SP.Stress, plastic_flow_direction, delta_Gamma, MatProp);
  }
...
static double compute_yield_surface(double relative_stress_norm, double EPS_n,
                                    Material MatProp) {
  ...
  if (MatProp.Hardening_Hughes) {
    ...
    return relative_stress_norm -
           sqrt(2. / 3.) * (Sigma_y + Hardening_modulus * theta * EPS_n);
  } else {
...
static double compute_increment_flow_rule(double Phi_tr, Material MatProp) {
  double nu = MatProp.nu; /* Poisson modulus */
  double E = MatProp.E;   /* Elastic modulus */
  double G = E / (2 * (1 + nu));
  ...
  if (MatProp.Hardening_Hughes) {
    ...
    if (MatProp.Viscous_regularization) {
      ...
      return (Phi_tr / (2 * G)) /
             (fluidity_param / (DT * 2 * G) + 1 + Hardening_modulus / (3 * G));
    } else {
...
static double update_equivalent_plastic_strain(double EPS_n,
                                               double delta_Gamma) {
...
static void
compute_increment_plastic_strain_tensor(double *Increment_E_plastic,
                                        double *plastic_flow_direction,
                                        double delta_Gamma) {
