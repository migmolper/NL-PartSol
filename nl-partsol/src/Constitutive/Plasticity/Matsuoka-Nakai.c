...
*/
{
  ...
  */
  ...
  // Plastic (monolithic solver with line search)
    ...
    STATUS = __d_G_d_stress(d_G_d_stress, T_tr, I1, I2, I3, c0, kappa_n[1], pa, m);
    if (STATUS == EXIT_FAILURE) {
    ...
    STATUS = __residual(Residual_k1, &Norm_Residual_1, E_hencky_trial,E_hencky_trial, d_G_d_stress, kappa_n, kappa_n, F_0, delta_lambda_k0);
    if (STATUS == EXIT_FAILURE) {
    ...
    while (fabs(F_k1 / F_0) >= TOL) {
      ...
      STATUS = __d_F_d_stress(d_F_d_stress, T_k1, I1, I2, I3, c0, kappa_k1[0], pa, m);
      if (STATUS == EXIT_FAILURE) {
        fprintf(stderr, "" RED "Error in __d_F_d_stress" RESET "\n");
        return EXIT_FAILURE;
      }
      STATUS = __d_F_d_kappa_phi(&d_F_d_kappa_phi,I1,I3, c0,m, pa, kappa_k1[0]);
      if (STATUS == EXIT_FAILURE) {
      ...
      STATUS = __dd_G_dd_stress(dd_G_dd_stress, T_k1, kappa_k1[1], I1, I2, I3, m, pa, c0);
      if (STATUS == EXIT_FAILURE) {
        fprintf(stderr, "" RED "Error in __dd_G_dd_stress" RESET "\n");
        return EXIT_FAILURE;
      }
      STATUS = __dd_G_d_stress_d_kappa_psi(dd_G_d_stress_d_kappa_psi, T_k1, I1, I3,m, pa, c0, kappa_k1[1]); 
      if (STATUS == EXIT_FAILURE) {
        fprintf(stderr, "" RED "Error in __dd_G_d_stress_d_kappa_psi" RESET "\n");
        return EXIT_FAILURE;
      }

      // Assemble tangent matrix
      STATUS = __tangent_matrix(Tangent_Matrix, CC,d_F_d_stress, d_F_d_kappa_phi,
                          d_G_d_stress, dd_G_dd_stress, dd_G_d_stress_d_kappa_psi, 
                          d_kappa_phi_d_stress, d_kappa_phi_d_lambda, alpha, delta_lambda_k1);
      if (STATUS == EXIT_FAILURE) {
      ...
      if (STATUS == EXIT_FAILURE) {
        fprintf(stderr, "" RED "Error in __E_hencky (line search loop)" RESET "\n");
        return EXIT_FAILURE;
      }

      STATUS = __kappa(kappa_hat, a, Lambda_k2, I1, alpha);
      if (STATUS == EXIT_FAILURE) {
        fprintf(stderr, "" RED "Error in __kappa (line search loop)" RESET "\n");
        return EXIT_FAILURE;
      }

      STATUS = __d_G_d_stress(d_G_d_stress, T_k2, I1, I2, I3, c0, kappa_k2[1], pa, m);
      if (STATUS == EXIT_FAILURE) {
        fprintf(stderr, "" RED "Error in __d_G_d_stress (line search loop)" RESET "\n");
        return EXIT_FAILURE;
      }

      F_k2 = __F(c0,kappa_k2[0], pa, I1, I2, I3, m);

      STATUS = __residual(Residual_k2, &Norm_Residual_2, E_hencky_trial,E_hencky_k2, d_G_d_stress, kappa_k2,kappa_hat, F_k2, delta_lambda_k2);
      if (STATUS == EXIT_FAILURE) {
        fprintf(stderr, "" RED "Error in __residual (line search loop)" RESET "\n");
        return EXIT_FAILURE;
      }
      ...
      if (Iter_k1 == MaxIter_k1)
      {
        ...
        if (rcond < 1E-10) {
          fprintf(stderr, "%s: %s %i, %s: %e\n", "Error in Frictional_Monolithic",
            "Tangent_Matrix is near to singular matrix for particle",Particle_Idx, "rcond", rcond);
        }
        break;
      }
...
static int __tangent_matrix(double *Tangent_Matrix, const double * CC,
                             ...
                             const double *d_G_d_stress, const double *dd_G_dd_stress,
                             const double *dd_G_d_stress_d_kappa2, const double *d_kappa_phi_d_stress, 
  ...
  Tangent_Matrix[3] = alpha * delta_lambda_k * dd_G_d_stress_d_kappa2[0];
  ...
  Tangent_Matrix[8] = alpha * delta_lambda_k * dd_G_d_stress_d_kappa2[1];
  ...
  Tangent_Matrix[13] = alpha * delta_lambda_k * dd_G_d_stress_d_kappa2[2];
