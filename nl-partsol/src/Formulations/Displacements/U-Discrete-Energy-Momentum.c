...
static void assemble_Nodal_Tangent_Stiffness_Material(Matrix, Mask, Particle,
                                                      Mesh);
static Tensor compute_stiffness_density(Tensor, Tensor, Tensor, double,
                                        Material);
...
void U_Discrete_Energy_Momentum(Mesh FEM_Mesh, Particle MPM_Mesh,
  ...
  */
  ...
  for (int TimeStep = InitialStep; TimeStep < NumTimeStep; TimeStep++) {
    ...
    */
    ...
    print_Status("*************************************************", TimeStep);
    print_Status("Six step : Compute increment of velocity ... WORKING",
                 TimeStep);
    /*
      Once the equilibrium is reached, obtain the increment of nodal velocity
    */
    D_Velocity =
        compute_Nodal_D_Velocity(Velocity, D_Displacement, DeltaTimeStep);
    print_Status("DONE !!!", TimeStep);
    ...
    print_Status("Seven step : Update particles lagrangian ... WORKING",
                 TimeStep);
    /*
          Update Lagrangians with D_Displacement and reload the connectivity
       information for each particle
    */
    update_Particles(D_Displacement, D_Velocity, MPM_Mesh, FEM_Mesh,
                     ActiveNodes, DeltaTimeStep);
    print_Status("DONE !!!", TimeStep);
    ...
    */
    if (TimeStep % ResultsTimeStep == 0) {
      ...
      // Reactions,TimeStep, ResultsTimeStep);
...
static Matrix compute_Nodal_Momentum(Particle MPM_Mesh, Mesh FEM_Mesh,
                                     Mask ActiveNodes)
/*
...
/*
   ...
   linearized and all the dof split the velocity array in n components like : |
   M 0 |   |V.x|   | p.x | | 0 M | * |V.y| = | p.y |
*/
{
...
static void imposse_Nodal_Velocity(Mesh FEM_Mesh, Matrix Velocity,
...
//        C_AB = compute_stiffness_density(GRADIENT_pA, GRADIENT_pB, F_n12_p, J_p, MatProp_p);
        ...
        /*
          ...
          freedom
        */
        Material_AB = compute_Nodal_Tangent_Stiffness_Material(
            F_n12_p, C_AB, transpose_F_n12_p);
...
static Tensor compute_stiffness_density(Tensor GRADIENT_pA, Tensor GRADIENT_pB,
  ...
  if (strcmp(MatProp_p.Type, "Saint-Venant-Kirchhoff") == 0) {
    C_AB = compute_stiffness_density_Saint_Venant_Kirchhoff(
        GRADIENT_pA, GRADIENT_pB, MatProp_p);
  } else if (strcmp(MatProp_p.Type, "Neo-Hookean-Wriggers") == 0) {

    Tensor C_p = right_Cauchy_Green__Particles__(F_p);

    C_AB = compute_material_stiffness_density_Neo_Hookean_Wriggers(
        GRADIENT_pA, GRADIENT_pB, C_p, J_p, MatProp_p);
...
static void
solve_non_reducted_system(Matrix D_Displacement, Matrix Tangent_Stiffness,
                          Matrix Effective_Mass, Matrix Residual, double Dt)
/*
...
*/
{
  ...
  */
  for (int idx_AB_ij = 0; idx_AB_ij < Order * Order; idx_AB_ij++) {
    K_Global.nV[idx_AB_ij] = (2 / DSQR(Dt)) * Effective_Mass.nV[idx_AB_ij] +
...
static Matrix compute_Nodal_D_Velocity(Matrix Velocity, Matrix D_Displacement,
                                       double Dt) {
  int Nnodes_mask = Velocity.N_rows;
  int Ndim = NumberDimensions;
  ...
  */
  for (int idx_A_i = 0; idx_A_i < Nnodes_mask * Ndim; idx_A_i++) {
    D_Velocity.nV[idx_A_i] =
        2 * (D_Displacement.nV[idx_A_i] / Dt - Velocity.nV[idx_A_i]);
  }
      ...
      */
      for (int i = 0; i < Ndim; i++) {
        MPM_Mesh.Phi.vel.nM[p][i] +=
            ShapeFunction_pI * D_Velocity.nM[A_mask][i];
