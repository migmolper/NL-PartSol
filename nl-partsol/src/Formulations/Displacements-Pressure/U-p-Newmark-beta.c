...
static Matrix assemble_Tangent_Stiffness(Mask, Particle, Mesh, double,
                                         Newmark_parameters);
...
void Up_Newmark_beta_Finite_Strains(Mesh FEM_Mesh, Particle MPM_Mesh,
  ...
  */
  ...
  for (unsigned TimeStep = InitialStep; TimeStep < NumTimeStep; TimeStep++) {
    ...
    while (Convergence == false) {
      ...
      if (Convergence == false) {
        ...
        Tangent_Stiffness = assemble_Tangent_Stiffness(
            ActiveNodes, MPM_Mesh, FEM_Mesh, epsilon, Params);
    ...
    /*
      Outputs
    */
    output_selector(Up_n, D_Up, MPM_Mesh, FEM_Mesh, ActiveNodes, TimeStep,
                    ResultsTimeStep);
      ...
      for (int B = 0; B < Nodes_p.NumberNodes; B++) {
        ...
        */
        for (int i = 0; i < Ndim; i++) {
          Effective_MassMatrix.nM[A_mask * Ndim + i][B_mask * Ndim + i] +=
...
/*
  ...
  components like : | M 0 | *  |d2 (u.x) dt2| = |m * d2 (u.x) dt2| | 0 M |_IJ
  |d2 (u.y) dt2|_J |m * d2 (u.y) dt2|_I

  -> The nodal Velocity/First time derivative of the pore water pressure.
  The operation is linearized and all the dof split the solution array in n
  components like : | M 0 | *   |d (u.x) dt| = |m * d (u.x) dt| | 0 M |_IJ  |d
  (u.y) dt|_J |m * d (u.y) dt|_I

  -> The nodal displacement/pore water pressure.
  The operation is linearized and all the dof split the solution array in n
  components like : | M 0 | *  |u.x| = |m * u.x| | 0 M |_IJ |u.y|_J |m * u.y|_I
...
*/
{
    ...
    */
    ...
    for (int A = 0; A < Nodes_p.NumberNodes; A++) {
      ...
      */
      ...
      Up_n.lambda_pressure.nV[A_mask] +=
          m_p * Na_p * MPM_Mesh.Phi.lambda_pressure_n.nV[p];
    }
  ...
  */
  for (int A = 0; A < Nnodes_mask; A++) {
    Up_n.lambda_pressure.nV[A] =
        Up_n.lambda_pressure.nV[A] / Lumped_MassMatrix.nV[A];
  }
...
static Nodal_Field initialise_Nodal_Increments(Nodal_Field Up_n,
        ...
        if (FEM_Mesh.Bounds.BCC_i[i].Dir[k * NumTimeStep + TimeStep] == 1) {
          ...
          */
          ...
          for (int t = 0; t < TimeStep; t++) {
            ...
            Up_n.d2_U_dt2.nM[Id_BCC_mask][k] +=
                alpha_1 * D_U_value_It -
                alpha_2 * Up_n.d_U_dt.nM[Id_BCC_mask][k] -
                (alpha_3 + 1) * Up_n.d2_U_dt2.nM[Id_BCC_mask][k];
            Up_n.d_U_dt.nM[Id_BCC_mask][k] +=
                alpha_4 * D_U_value_It +
                (alpha_5 - 1) * Up_n.d_U_dt.nM[Id_BCC_mask][k] +
                alpha_6 * Up_n.d2_U_dt2.nM[Id_BCC_mask][k];
          }

          /*
            Initialise increments using newmark and the U of the boundary
            condition
          */
          D_Up.U.nM[Id_BCC_mask][k] =
              FEM_Mesh.Bounds.BCC_i[i].Value[k].Fx[TimeStep];
          D_Up.d2_U_dt2.nM[Id_BCC_mask][k] =
              alpha_1 * D_Up.U.nM[Id_BCC_mask][k] -
              alpha_2 * Up_n.d_U_dt.nM[Id_BCC_mask][k] -
              (alpha_3 + 1) * Up_n.d2_U_dt2.nM[Id_BCC_mask][k];
          D_Up.d_U_dt.nM[Id_BCC_mask][k] =
              alpha_4 * D_Up.U.nM[Id_BCC_mask][k] +
              (alpha_5 - 1) * Up_n.d_U_dt.nM[Id_BCC_mask][k] +
              alpha_6 * Up_n.d2_U_dt2.nM[Id_BCC_mask][k];
        }
      }
...
static void update_Local_State(Nodal_Field D_Up, Mask ActiveNodes,
...
Define tributary nodes of the particle
    ...
    */
    ...
    D_lambda_pressure_n1 =
        get_P_set_field_Up(D_Up.lambda_pressure, Nodes_p, ActiveNodes);
...
//                                                     DF_p, dFdt_n_p);
    ...
    */
    ...
    lambda_pressure_n1 =
        lambda_pressure_n +
        interpolate_scalar_magnitude__MeshTools__(D_lambda_pressure_n1, N_p);
  ...
  */
  for (int p = 0; p < Np; p++) {
    ...
    /*
      ...
      integration rule.
    */
    Stress_integration__Constitutive__(p, MPM_Mesh, MatProp_p);
  }
}

/*********************************************************************/

static Matrix get_U_set_field_Up(Matrix Field_U, Element Nodes_p,
                                 Mask ActiveNodes) {
...
static Matrix get_P_set_field_Up(Matrix Field_lambda_pressure, Element Nodes_p,
                                 Mask ActiveNodes) {
  int Nnodes = Nodes_p.NumberNodes;
  int Ndim = NumberDimensions;
...
static Matrix compute_Residual(Nodal_Field up_n, Nodal_Field D_up, Mask ActiveNodes,
  ...
  compute_Volumetric_Constrain_Forces(Residual, ActiveNodes, MPM_Mesh,
                                      FEM_Mesh);
...
static void compute_Inertial_Forces(Nodal_Field D_up, Nodal_Field up_n,
  ...
  for (int idx_A = 0; idx_A < Nnodes_mask; idx_A++) {
    for (int i = 0; i < Ndim; i++) {
      for (int idx_B = 0; idx_B < Nnodes_mask; idx_B++) {
        Residual.nM[idx_A][i] +=
            Effective_Mass.nM[idx_A * Ndim + i][idx_B * Ndim + i] *
            Acceleration_n1.nM[idx_B][i];
      }
    }
  }
...
static void compute_Internal_Forces(Matrix Residual, Mask ActiveNodes,
                                    Particle MPM_Mesh, Mesh FEM_Mesh)
/*
...
*/
{
  ...
  */
  for (int p = 0; p < Np; p++) {
    ...
    */
    P_constrained_p =
        memory_to_tensor__TensorLib__(MPM_Mesh.Phi.Stress.nM[p], 2);
    ...
    for (int A = 0; A < NumNodes_p; A++) {
      ...
      */
      InternalForcesDensity_Ap =
          vector_linear_mapping__TensorLib__(P_constrained_p, GRADIENT_pA);
...
static void
compute_Volumetric_Constrain_Forces(Matrix Residual, Mask ActiveNodes,
                                    Particle MPM_Mesh, Mesh FEM_Mesh)
/*!
...
static void compute_Nodal_Nominal_traction_Forces(Matrix Residual,
                                                  Mask ActiveNodes,
                                                  Particle MPM_Mesh,
...
static Matrix assemble_Tangent_Stiffness(Mask ActiveNodes, Particle MPM_Mesh,
...
//    J_p = I3__TensorLib__(F_n1_p);
    ...
    for (int A = 0; A < NumNodes_p; A++) {
      ...
      */
      ...
      GRADIENT_N_pA =
          vector_linear_mapping__TensorLib__(transpose_F_n_p, gradient_N_pA);
      FmTGRADIENT_N_pA =
          vector_linear_mapping__TensorLib__(FmT_n1_p, GRADIENT_N_pA);
      ...
      */
      ...
      for (int B = 0; B < NumNodes_p; B++) {
        ...
        */
        ...
        GRADIENT_N_pB =
            vector_linear_mapping__TensorLib__(transpose_F_n_p, gradient_N_pB);
        FmTGRADIENT_N_pB =
            vector_linear_mapping__TensorLib__(FmT_n1_p, GRADIENT_N_pB);
        ...
        */
        for (int i = 0; i < Ndim; i++) {
          ...
          Tangent_Stiffness.nM[A_mask * Ndof + i][B_mask * Ndof + Ndim] -=
              J_p * FmTGRADIENT_N_pA.n[i] * N_pB * V0_p;
...
static void solve_system(Nodal_Field D_up, Matrix Tangent_Stiffness,
                         Matrix Residual)
/*
...
static void update_Newmark_Nodal_Increments(Nodal_Field D_up, Nodal_Field up_n,
                                            Newmark_parameters Params) {
  ...
  */
  for (int A = 0; A < Nnodes; A++) {
    for (int i = 0; i < Ndim; i++) {
      D_up.d2_U_dt2.nM[A][i] = alpha_1 * D_up.U.nM[A][i] -
                               alpha_2 * up_n.d_U_dt.nM[A][i] -
                               (alpha_3 + 1) * up_n.d2_U_dt2.nM[A][i];
      D_up.d_U_dt.nM[A][i] = alpha_4 * D_up.U.nM[A][i] +
                             (alpha_5 - 1) * up_n.d_U_dt.nM[A][i] +
                             alpha_6 * up_n.d2_U_dt2.nM[A][i];
    }
  }
}
...
static void update_Particles(Nodal_Field D_up, Particle MPM_Mesh, Mesh FEM_Mesh,
                             Mask ActiveNodes) {
  int Ndim = NumberDimensions;
  int Ndof = NumberDimensions + 1;
  int Np = MPM_Mesh.NumGP;
    ...
    */
    for (int A = 0; A < Nodes_p.NumberNodes; A++) {
      ...
      */
      ...
      MPM_Mesh.Phi.lambda_pressure_n.nV[p] +=
          N_pI * D_up.lambda_pressure.nV[A_mask];
    }
...
static void output_selector(Nodal_Field D_up, Nodal_Field up_n,
  ...
  */
  ...
  //    up_n1.d2_value_dt2.nV[I] = D_up.d2_value_dt2.nV[I] +
  //    up_n.d2_value_dt2.nV[I];
  //  }
  ...
  */
  if (TimeStep % ResultsTimeStep == 0) {
    ...
    //    nodal_results_upw_vtk__InOutFun__(up_n1.value, up_n1.d_value_dt,
