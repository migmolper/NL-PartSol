...
void initialize__T3__(Particle MPM_Mesh, Mesh FEM_Mesh) {
...
Matrix dN_Ref__T3__(Matrix X_e) {
...
static Matrix F_Ref__T3__(Matrix Xi, Matrix Element)
/*
...
Matrix dN__T3__(Matrix Xi, Matrix Element)
/*
...
static Matrix Xi_to_X__T3__(Matrix Xi, Matrix Element)
/*
This function evaluate the position of the GP in the element, and get it global
coordiantes
 */
{
  int Ndim = NumberDimensions;
...
static void X_to_Xi__T3__(Matrix X_EC_GP, Matrix X_GC_GP, Matrix Element_GC_Nod)
/*
...
*/
{
  ...
  X_EC_GP = Newton_Rapson(Xi_to_X__T3__, Element_GC_Nod, F_Ref__T3__,
                          Element_GC_Nod, X_GC_GP, X_EC_GP);
}
...
void element_to_particles__T3__(Matrix X_p, Mesh FEM_Mesh, int GPxElement) {
  ...
  case 9:
    Xi_p.nM[0][0] = 0.11111111111;
    Xi_p.nM[0][1] = 0.11111111111;

    Xi_p.nM[1][0] = 0.44444444444;
    Xi_p.nM[1][1] = 0.11111111111;

    Xi_p.nM[2][0] = 0.77777777777;
    Xi_p.nM[2][1] = 0.11111111111;

    Xi_p.nM[3][0] = 0.22222222222;
    Xi_p.nM[3][1] = 0.22222222222;

    Xi_p.nM[4][0] = 0.55555555555;
    Xi_p.nM[4][1] = 0.22222222222;

    Xi_p.nM[5][0] = 0.11111111111;
    Xi_p.nM[5][1] = 0.44444444444;

    Xi_p.nM[6][0] = 0.44444444444;
    Xi_p.nM[6][1] = 0.44444444444;

    Xi_p.nM[7][0] = 0.22222222222;
    Xi_p.nM[7][1] = 0.55555555555;

    Xi_p.nM[8][0] = 0.11111111111;
    Xi_p.nM[8][1] = 0.77777777777;
    break;

  default:
    fprintf(stderr, "%s : %s \n", "Error in element_to_particles__T3__()",
...
double min_DeltaX__T3__(ChainPtr Element_Connectivity, Matrix Coordinates) {
  ...
  Matrix X_eval =
      allocZ__MatrixLib__(1, Ndim); /* Where to evaluate the shape function */
  X_eval.nV[0] = 0.0;
  X_eval.nV[1] = 0.0;
  ...
  */
  for (int j = 0; j < NumNodesElem; j++) {
    MinElementSize =
        DMIN(MinElementSize, 1 / sqrt(dNdx.nM[j][0] * dNdx.nM[j][0] +
                                      dNdx.nM[j][1] * dNdx.nM[j][1]));
  }
...
double volume__T3__(Matrix Element) {
  int Ndim = NumberDimensions;
  double J_i;
  double Vol = 0;
  ...
  double table_X[3][2] = {{0.600000000000000, 0.200000000000000},
