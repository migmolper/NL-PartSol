...
void initialize__H8__(Particle MPM_Mesh, Mesh FEM_Mesh) {
...
Matrix N__H8__(Matrix Xi) {
  ...
  N_ref.nV[0] =
      0.125 *
      DMIN(8, DMAX(0, (1. - Xi.nV[0]) * (1. - Xi.nV[1]) * (1. - Xi.nV[2])));
  N_ref.nV[1] =
      0.125 *
      DMIN(8, DMAX(0, (1. + Xi.nV[0]) * (1. - Xi.nV[1]) * (1. - Xi.nV[2])));
  N_ref.nV[2] =
...
Matrix dN_Ref__H8__(Matrix Xi) {
  ...
  /* Fill the matrix */
  dNdX_ref.nM[0][0] =
      -0.125 * DMIN(4, DMAX(0, (1. - Xi.nV[1]) * (1. - Xi.nV[2])));
  dNdX_ref.nM[0][1] =
      -0.125 * DMIN(4, DMAX(0, (1. - Xi.nV[0]) * (1. - Xi.nV[2])));
  dNdX_ref.nM[0][2] =
      -0.125 * DMIN(4, DMAX(0, (1. - Xi.nV[0]) * (1. - Xi.nV[1])));
...
static void X_to_Xi__H8__(Matrix Xi, Matrix X, Matrix Element)
/*
...
bool in_out__H8__(Matrix X, Matrix Element) {
  double min[3] = {Element.nM[0][0], Element.nM[0][1], Element.nM[0][2]};
  double max[3] = {Element.nM[0][0], Element.nM[0][1], Element.nM[0][2]};
  ...
  if ((X.nV[0] <= max[0]) && (X.nV[0] >= min[0]) && (X.nV[1] <= max[1]) &&
      (X.nV[1] >= min[1]) && (X.nV[2] <= max[2]) && (X.nV[2] >= min[2])) {

    Xi = allocZ__MatrixLib__(3, 1);

    X_to_Xi__H8__(Xi, X, Element);
...
void element_to_particles__H8__(Matrix X_p, Mesh FEM_Mesh, int GPxElement) {
  int Ndim = NumberDimensions;
  ...
  default:
    fprintf(stderr, "%s : %s \n", "Error in element_to_particles__H8__()",
...
double min_DeltaX__H8__(ChainPtr Element_Connectivity, Matrix Coordinates) {
  ...
  */
  ...
  for (int k = 0; k < 4; k++) {
    ...
    for (int l = 0; l < Ndim; l++) {
      sqr_lenght +=
          DSQR(Coordinates.nM[Node_k4][l] - Coordinates.nM[Node_k][l]);
    }
...
double volume__H8__(Matrix Element) {
  int Ndim = NumberDimensions;
  double J_i;
  double Vol = 0;
...
void local_search__H8__(Particle MPM_Mesh, Mesh FEM_Mesh) {
