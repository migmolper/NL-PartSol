...
void initialize__GIMP__(Particle MPM_Mesh, Mesh FEM_Mesh) {
  ...
  /* Variables to store the conectivity and coordinates of the element of the
   * particle */
  ...
  for (int i = 0; i < MPM_Mesh.NumGP; i++) {
    ...
    for (int j = 0; j < FEM_Mesh.NumElemMesh; j++) {
      ...
      if (FEM_Mesh.In_Out_Element(X_p, Elem_p_Coordinates)) {
        ...
        MPM_Mesh.I0[i] = get_closest_node__MeshTools__(X_p, Elem_p_Connectivity,
        ...
        asign_to_nodes__Particles__(i, j, MPM_Mesh.I0[i], MPM_Mesh.ListNodes[i],
                                    FEM_Mesh);
...
static Matrix F_Ref__GIMP__(Matrix X_NC_GP, Matrix X_GC_Nodes)
/*
...
static Matrix Xi_to_X__GIMP__(Matrix Xi, Matrix Element)
/*
...
static void X_to_Xi__GIMP__(Matrix Xi, Matrix X, Matrix Element)
/*
...
ChainPtr tributary__GIMP__(Matrix Xi_p, int Elem_GP, Matrix lp, Mesh FEM_Mesh) {
  ...
  /* Check if I am in the 1º Quadrant */
    ...
    if ((fabs(Xi_p.nV[0]) >= Dist[0]) && (fabs(Xi_p.nV[1]) <= Dist[1])) {
      ...
      ChainElements =
          intersection__SetLib__(FEM_Mesh.NodeNeighbour[NodesElem[2]],
    ...
    } else if ((fabs(Xi_p.nV[0]) <= Dist[0]) && (fabs(Xi_p.nV[1]) >= Dist[1])) {
      /* Generate the list of Elements whose nodes contributes to the GP */
      ChainElements =
          intersection__SetLib__(FEM_Mesh.NodeNeighbour[NodesElem[2]],
  ...
  /* Check if I am in the 2º Quadrant */
    ...
    if ((fabs(Xi_p.nV[0]) <= Dist[0]) && (fabs(Xi_p.nV[1]) >= Dist[1])) {
      ...
      /* Generate the list of Elements whose nodes contributes to the GP */
      ChainElements =
          intersection__SetLib__(FEM_Mesh.NodeNeighbour[NodesElem[3]],
  ...
  /* Check if I am in the 3º Quadrant */
    ...
    if ((fabs(Xi_p.nV[0]) >= Dist[0]) && (fabs(Xi_p.nV[1]) <= Dist[1])) {
      ...
      /* Generate the list of Elements whose nodes contributes to the GP */
      ChainElements =
          intersection__SetLib__(FEM_Mesh.NodeNeighbour[NodesElem[0]],
                                 FEM_Mesh.NodeNeighbour[NodesElem[1]]);
      Num_Elem = lenght__SetLib__(ChainElements);
      Tributary_Elements = set_to_memory__SetLib__(ChainElements, Num_Elem);
      free__SetLib__(&ChainElements);
    ...
    } else if ((fabs(Xi_p.nV[1]) >= Dist[1]) && (fabs(Xi_p.nV[0]) >= Dist[0])) {
      /* Generate the list of Elements whose nodes contributes to the GP */
  ...
  /* Check if it I am the 4º Quadrant */
    ...
    printf("%s : (%f;%f) \n", "Natural coordinates of the GP", Xi_p.nV[0],
           Xi_p.nV[1]);
    exit(EXIT_FAILURE);
  }
