...
void initialize__T4__(Particle MPM_Mesh, Mesh FEM_Mesh) {
...
Matrix N__T4__(Matrix Xi) {
  ...
  N_ref.nV[1] = Xi.nV[1];
  N_ref.nV[2] = Xi.nV[2];
  N_ref.nV[3] = 1. - Xi.nV[0] - Xi.nV[1] - Xi.nV[2];
...
/*
  ...
  - Matrix Element : Coordinates of the element (NumNodesElem x
  NumberDimensions)
*/
{
...
static void X_to_Xi__T4__(Matrix Xi, Matrix X, Matrix Element)
/*
...
bool in_out__T4__(Matrix X, Matrix Element) {
  ...
  if ((X.nV[0] <= max[0]) && (X.nV[0] >= min[0]) && (X.nV[1] <= max[1]) &&
    ...
    X_to_Xi__T4__(Xi, X, Element);

    if ((Xi.nV[0] >= 0.0) && (Xi.nV[1] >= 0.0) && (Xi.nV[2] >= 0.0) &&
...
void element_to_particles__T4__(Matrix X_p, Mesh FEM_Mesh, int GPxElement)
/*
...
double min_DeltaX__T4__(ChainPtr Element_Connectivity, Matrix Coordinates) {
  ...
  */
  ...
  for (int k = 0; k < 2; k++) {
    ...
    Node_k = Element_Connectivity->Idx;
    Node_k2 = Element_Connectivity->next->next->Idx;

    sqr_lenght = 0.0;

    for (int l = 0; l < Ndim; l++) {
      sqr_lenght +=
          DSQR(Coordinates.nM[Node_k2][l] - Coordinates.nM[Node_k][l]);
    }
...
double volume__T4__(Matrix Element) {
  int Ndim = NumberDimensions;
  double J_i;
  double Vol = 0;
...
void local_search__T4__(Particle MPM_Mesh, Mesh FEM_Mesh) {
