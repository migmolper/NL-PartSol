...
static void Fill_Linear_Conectivity(char *, Mesh_Information, ChainPtr *,
                                    int *);
static void Fill_Quadratic_Conectivity_Triangle(char *, Mesh_Information,
                                                ChainPtr *, int *, int *);
static void Fill_Quadratic_Conectivity_Tetrahedra(char *, Mesh_Information,
                                                  ChainPtr *, int *, int *);
...
Mesh ReadGidMesh__MeshTools__(char *MeshName) {
  ...
  */
  if ((strcmp(Mesh_Info.ElemType, "Triangle") == 0) &&
    ...
    Fill_Linear_Conectivity(MeshName, Mesh_Info, GID_Mesh.Connectivity,
                            GID_Mesh.NumNodesElem);
  } else if ((strcmp(Mesh_Info.ElemType, "Triangle") == 0) &&
    ...
    Fill_Linear_Conectivity(MeshName, Mesh_Info, GID_Mesh.Connectivity,
                            GID_Mesh.NumNodesElem);
  } else if ((strcmp(Mesh_Info.ElemType, "Tetrahedra") == 0) &&
             (Mesh_Info.NumNodesElem == 4)) {
    ...
    GID_Mesh.Vol_Patch_n1 =
        (double *)Allocate_ArrayZ(Mesh_Info.NumElemMesh, sizeof(double));
    Fill_Coordinates(MeshName, Mesh_Info, GID_Mesh.Coordinates);
    Fill_Quadratic_Conectivity_Tetrahedra(
        MeshName, Mesh_Info, GID_Mesh.Connectivity, GID_Mesh.Idx_Patch,
        GID_Mesh.NumNodesElem);
  } else if ((strcmp(Mesh_Info.ElemType, "Hexahedra") == 0) &&
    ...
    sprintf(Error_message,
            "Non suported element -> TypeElem : %s ; NumNodesElem : %i\n",
...
static void Fill_Coordinates(char *MeshName, Mesh_Information Mesh_Info,
                             Matrix Coordinates) {
...
static void Fill_Linear_Conectivity(char *MeshName, Mesh_Information Mesh_Info,
...
static void Fill_Quadratic_Conectivity_Triangle(
  ...
  Fill_Linear_Conectivity(MeshName, Quadratic_Mesh_Info, Quadratic_Connectivity,
                          NumNodesElem);
...
static void standard_error(int Num_line, char *MeshName) {
  fprintf(stderr,
          "ReadGidMesh__MeshTools__ : Error in line %i while reading %s \n",
          Num_line, MeshName);
  fprintf(stderr, "%s \n", Error_message);
  exit(EXIT_FAILURE);
}
