...
Matrix Identity__MatrixLib__(int Size) {
  Matrix Identity = allocZ__MatrixLib__(Size, Size);
...
*/
{
  ...
  if ((Rows > 1) && (Columns > 1) && (Columns == Rows)) {
    ...
    case 2:
      Det_out = In.nM[0][0] * In.nM[1][1] - In.nM[0][1] * In.nM[1][0];
      break;

    case 3:
      Det_out = In.nM[0][0] * In.nM[1][1] * In.nM[2][2] -
...
*/
{
  ...
  double *WORK_DGETRI =
      (double *)Allocate_Array(IMAX(1, LWORK), sizeof(double));
...
*/
{
  ...
  if ((Columns > 1) && (Rows > 1)) {
    if (M.nM == NULL) {
      fprintf(stderr, "%s : %s \n", "Error in transpose__MatrixLib__()",
              "NULL input !");
      exit(EXIT_FAILURE);
    }
...
Matrix vectorial_product__MatrixLib__(Matrix a, Matrix b) {
  ...
  if ((a.N_rows == 3) && (b.N_rows == 3) && (a.N_cols == 1) &&
      (b.N_cols == 1)) {
    ...
    fprintf(stderr, "%s : %s \n", "Error in vectorial_product__MatrixLib__()",
            "Incompatible shape of arrays !");
    exit(EXIT_FAILURE);
  }
...
*/
{
  ...
  /* Fail */
    ...
    fprintf(stderr, "%s : %s \n", "Error in dyadic_product__MatrixLib__()",
            "Wrong input matrix !");
    exit(EXIT_FAILURE);
  }
...
*/
{
  ...
  if ((A.N_cols == Incr.N_cols) && (A.N_rows == Incr.N_rows)) {
    ...
    /* Fail */
      ...
      fprintf(stderr, "%s : %s \n", "Error in increment__MatrixLib__()",
              "Wrong input matrix!");
      exit(EXIT_FAILURE);
    }
...
*/
{
  ...
  if ((A.N_cols == B.N_cols) && (A.N_rows == B.N_rows)) {
    ...
    /* Fail */
      ...
      fprintf(stderr, "%s : %s \n", "Error in addition__MatrixLib__()",
              "Wrong input matrix!");
      exit(EXIT_FAILURE);
    }
...
Matrix Eigen_Mat(Matrix In) {
  ...
  if ((Rows == Columns) && (Columns > 1) && (Rows > 1)) {
    ...
    case 2:
      ...
      Coeffs.nV[2] =
          In.nM[0][0] * In.nM[1][1] - In.nM[1][0] * In.nM[0][1]; /* c */
    ...
    case 3:
      ...
      Coeffs.nV[2] = /* c*x */
          +In.nM[0][0] * In.nM[2][2] + In.nM[1][1] * In.nM[2][2] +
          In.nM[0][0] * In.nM[1][1] - In.nM[1][0] * In.nM[0][1] -
          In.nM[2][1] * In.nM[1][2] - In.nM[2][0] * In.nM[0][2];
      Coeffs.nV[2] = /* d */
          -In.nM[0][0] * In.nM[1][1] * In.nM[2][2] -
          In.nM[0][1] * In.nM[1][2] * In.nM[2][0] -
          In.nM[0][2] * In.nM[1][0] * In.nM[2][1] +
