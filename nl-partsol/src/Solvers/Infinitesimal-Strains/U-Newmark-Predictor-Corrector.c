...
static Matrix compute_Velocity_Predictor(Particle, Mesh, Matrix, Matrix, double,
                                         double);
static void imposse_Velocity(Mesh, Matrix, int, int);
static Matrix compute_Velocity_Corrector(Mesh, Matrix, Matrix, Matrix, double,
                                         double);
...
void U_Newmark_Predictor_Corrector(Mesh FEM_Mesh, Particle MPM_Mesh,
  ...
  */
  ...
  for (int TimeStep = InitialStep; TimeStep < NumTimeStep; TimeStep++) {
    ...
    Velocity = compute_Velocity_Predictor(MPM_Mesh, FEM_Mesh, Velocity,
                                          Lumped_Mass, gamma, DeltaTimeStep);
    imposse_Velocity(FEM_Mesh, Velocity, TimeStep, NumTimeStep);
    ...
    Forces =
        compute_BodyForces(Forces, MPM_Mesh, FEM_Mesh, TimeStep, NumTimeStep);
    Forces =
        compute_ContacForces(Forces, MPM_Mesh, FEM_Mesh, TimeStep, NumTimeStep);
    Reactions = compute_Reactions(FEM_Mesh, Forces, TimeStep, NumTimeStep);
    print_Status("DONE !!!", TimeStep);
    ...
    print_Status("Four step : Update particles lagrangian ... WORKING",
                 TimeStep);
    update_Particles(MPM_Mesh, FEM_Mesh, Lumped_Mass, Velocity, Forces,
                     DeltaTimeStep);
    ...
    for (int j = 0; j < NumNodesBound; j++) {
      ...
      for (int k = 0; k < NumDimBound; k++) {
        ...
        if (FEM_Mesh.Bounds.BCC_i[i].Dir[k * NumTimeStep + TimeStep] == 1) {
          ...
          V_I.nM[Id_BCC][k] = FEM_Mesh.Bounds.BCC_i[i].Value[k].Fx[TimeStep];
        }
      }
    }
  }
...
*/
{
  ...
  for (int I = 0; I < Nnodes; I++) {
    if (FEM_Mesh.ActiveNode[I]) {
      for (int i = 0; i < Ndim; i++) {
        Velocity.nM[I][i] +=
            gamma * DeltaTimeStep * Forces.nM[I][i] / Lumped_Mass.nV[I];
      }
    }
  }
...
static void update_Particles(Particle MPM_Mesh, Mesh FEM_Mesh,
                             Matrix Lumped_Mass, Matrix Velocity, Matrix Forces,
                             double DeltaTimeStep) {
      ...
      if (mass_I > 0) {
        for (int i = 0; i < Ndim; i++) {
          ...
          D_U_pI = ShapeFunction_pI * DeltaTimeStep * Velocity.nM[Ip][i] +
                   ShapeFunction_pI * 0.5 * pow(DeltaTimeStep, 2) *
                       Forces.nM[Ip][i] / mass_I;
...
static void update_LocalState(Matrix V_I, Particle MPM_Mesh, Mesh FEM_Mesh,
  ...
  for (int p = 0; p < Np; p++) {
    ...
    Rate_Strain_p =
        rate_inifinitesimal_Strain__Particles__(Nodal_Velocity_p, Gradient_p);
    Strain_p =
        infinitesimal_Strain__Particles__(Strain_p, Rate_Strain_p, TimeStep);
    ...
    Stress_p =
        explicit_integration_stress__Particles__(p, MPM_Mesh, Material_p);
