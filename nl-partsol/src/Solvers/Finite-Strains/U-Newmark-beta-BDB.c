...
static void imposse_Nodal_Kinetics(Mesh, Matrix, Matrix, Mask, int, int);
...
static Matrix compute_Nodal_Residual(Matrix, Matrix, Matrix, Matrix, Matrix,
                                     Newmark_parameters);
static bool check_convergence(Matrix, double, int, int, int);
static Matrix assemble_Nodal_Tangent_Stiffness(Mask, Particle, Mesh);
static void assemble_Nodal_Tangent_Stiffness_Geometric(Matrix, Mask, Particle,
                                                       Mesh);
static void assemble_Nodal_Tangent_Stiffness_Material(Matrix, Mask, Particle,
                                                      Mesh);
...
static void solve_non_reducted_system(Matrix, Matrix, Matrix, Matrix,
                                      Newmark_parameters);
static void solve_reducted_system(Mask, Matrix, Matrix, Matrix, Matrix,
                                  Newmark_parameters);
static void update_Particles(Matrix, Matrix, Matrix, Particle, Mesh, Mask,
                             Newmark_parameters);
static void output_selector(Particle, Mesh, Mask, Matrix, Matrix, Matrix,
                            Matrix, Matrix, Matrix, double, int, int);
/**************************************************************/

void U_Newmark_beta_Finite_Strains_BDB(Mesh FEM_Mesh, Particle MPM_Mesh,
  ...
  */
  ...
  for (int TimeStep = InitialStep; TimeStep < NumTimeStep; TimeStep++) {
    ...
    /*
             With the active set of nodes generate a mask to help the algorithm
       to compute the equilibrium only in the active nodes
    */
    ...
    /*
             Compute the effective mass matrix as a convex combination of the
       consistent mass matrix and the lumped mass matrix.
    */
    Effective_Mass =
    ...
    */
    Velocity =
        compute_Nodal_Velocity(Effective_Mass, MPM_Mesh, FEM_Mesh, ActiveNodes);
    Acceleration = compute_Nodal_Acceleration(Effective_Mass, MPM_Mesh,
                                              FEM_Mesh, ActiveNodes);

    imposse_Nodal_Kinetics(FEM_Mesh, Velocity, Acceleration, ActiveNodes,
    ...
    */
    ...
    while (Convergence == false) {
      ...
      */
      Forces = compute_Nodal_Forces(D_Displacement, ActiveNodes, MPM_Mesh,
      ...
      */
      Residual = compute_Nodal_Residual(Velocity, Acceleration, Forces,
                                        D_Displacement, Effective_Mass, Params);
      ...
      */
      if (Convergence == false) {
        ...
        */
        Tangent_Stiffness =
            assemble_Nodal_Tangent_Stiffness(ActiveNodes, MPM_Mesh, FEM_Mesh);
        ...
        */
        if ((Free_and_Restricted_Dofs.Nactivenodes - Ndim * Nactivenodes) ==
          ...
          solve_non_reducted_system(D_Displacement, Tangent_Stiffness,
                                    Effective_Mass, Residual, Params);
        } else {
          solve_reducted_system(Free_and_Restricted_Dofs, D_Displacement,
                                Tangent_Stiffness, Effective_Mass, Residual,
                                Params);
        }
    ...
    */
    update_Particles(D_Displacement, Velocity, Acceleration, MPM_Mesh, FEM_Mesh,
                     ActiveNodes, Params);
    print_Status("DONE !!!", TimeStep);
    ...
    /*
      Outputs
    */
    output_selector(MPM_Mesh, FEM_Mesh, ActiveNodes, Velocity, Acceleration,
                    D_Displacement, Forces, Reactions, Residual, DeltaTimeStep,
      ...
      for (int B = 0; B < Nodes_p.NumberNodes; B++) {
        ...
        */
        for (int i = 0; i < Ndof; i++) {
          ...
          */
          Effective_MassMatrix.nM[A_mask * Ndof + i][A_mask * Ndof + i] +=
              m_AB_p;
        }
      }
    }
  ...
  for (int A = 0; A < Order; A++) {
    for (int B = 0; B < Order; B++) {
      Effective_MassMatrix.nM[A][B] =
          ...
          (A == B) * epsilon * Lumped_MassMatrix.nM[A][B];
    }
  }
...
static Matrix compute_Nodal_Velocity(Matrix Mass, Particle MPM_Mesh,
...
static Matrix compute_Nodal_Acceleration(Matrix Mass, Particle MPM_Mesh,
                                         Mesh FEM_Mesh, Mask ActiveNodes)
/*
  Call the LAPACK solver to compute the nodal Acceleration. The operation is
  linearized and all the dof split the acceleration array in n components like :
  | M 0 |   |A.x|   | dt_p.x |
  | 0 M | * |A.y| = | dt_p.y |
*/
{
  ...
  */
  dgetrs_(&TRANS, &Order, &NRHS, Mass.nV, &LDA, IPIV, dt_Momentum.nV, &LDB,
          &INFO);
  free(IPIV);
...
static void imposse_Nodal_Kinetics(Mesh FEM_Mesh, Matrix Velocity,
                                   Matrix Acceleration, Mask ActiveNodes,
...
static void imposed_Nodal_Displacements(Matrix D_Displacement, Mask ActiveNodes,
    ...
    for (int A = 0; A < NumNodesBound; A++) {
      ...
      */
      ...
      for (int i_dim = 0; i_dim < NumDimBound; i_dim++) {
        ...
        if (FEM_Mesh.Bounds.BCC_i[i_boundary]
                .Dir[i_dim * NumTimeStep + TimeStep] == 1) {
          /*
             Assign the boundary condition
          */
          D_Displacement.nM[A_mask_BCC][i_dim] =
...
static Matrix compute_Nodal_Forces(Matrix D_Displacement, Mask ActiveNodes,
                                   Particle MPM_Mesh, Mesh FEM_Mesh,
                                   int TimeStep, int NumTimeStep) {
  int Ndim = NumberDimensions;
  int Nnodes_mask = ActiveNodes.Nactivenodes;
  ...
  */
  compute_Nodal_Internal_Forces(Forces, D_Displacement, ActiveNodes, MPM_Mesh,
                                FEM_Mesh);
...
static void compute_Nodal_Internal_Forces(Matrix Forces, Matrix D_Displacement,
...
static Matrix compute_Nodal_Residual(Matrix Velocity, Matrix Acceleration,
                                     Matrix Forces, Matrix D_Displacement,
  ...
  */
  for (int idx_B = 0; idx_B < Order; idx_B++) {
    Acceleration_n1.nV[idx_B] = alpha_1 * D_Displacement.nV[idx_B] -
                                alpha_2 * Velocity.nV[idx_B] -
                                alpha_3 * Acceleration.nV[idx_B];
  }
  /*
...
*/
{
  ...
  */
  assemble_Nodal_Tangent_Stiffness_Geometric(Tangent_Stiffness, ActiveNodes,
                                             MPM_Mesh, FEM_Mesh);

  /*
    Compute term related to the material non-linearities
  */
  assemble_Nodal_Tangent_Stiffness_Material(Tangent_Stiffness, ActiveNodes,
                                            MPM_Mesh, FEM_Mesh);
    ...
    for (int A = 0; A < NumNodes_p; A++) {
      ...
      */
      ...
      for (int B = 0; B < NumNodes_p; B++) {
        ...
        */
        GRADIENT_pA_o_GRADIENT_pB =
            dyadic_Product__TensorLib__(GRADIENT_pA, GRADIENT_pB);

        /*
          Get the nodal contribution
        */
        Geometric_AB_p =
            inner_product__TensorLib__(S_p, GRADIENT_pA_o_GRADIENT_pB);
        ...
        */
        for (int i = 0; i < Ndim; i++) {
          /*
            Compute the vectorized index
          */
          Tangent_Stiffness.nM[A_mask * Ndim + i][B_mask * Ndim + i] +=
              Geometric_AB_p * V0_p;
        }
      ...
      */
      ...
      for (int B = 0; B < NumNodes_p; B++) {
        ...
        */
        ...
        /*
            Add the geometric contribution to each dof for the assembling
           process
        */
        for (int i = 0; i < Ndim; i++) {
...
static Matrix compute_material_matrix_D(Tensor F_p, double J_p,
                                        Material MatProp_p) {
...
static Matrix compute_Nodal_BDB(Matrix D, Tensor GRADIENT_pA,
                                Tensor GRADIENT_pB, Tensor F_p) {
...
static void
solve_reducted_system(Mask Free_and_Restricted_Dofs, Matrix D_Displacement,
                      Matrix Tangent_Stiffness, Matrix Effective_Mass,
                      Matrix Residual, Newmark_parameters Params)
/*
  This function is deboted to update the vector with the nodal displacement by
  solving :
...
static void update_Particles(Matrix D_Displacement, Matrix Velocity,
                             Matrix Acceleration, Particle MPM_Mesh,
                             Mesh FEM_Mesh, Mask ActiveNodes,
                             Newmark_parameters Params) {
  int Ndim = NumberDimensions;
  int Np = MPM_Mesh.NumGP;
  ...
  for (int p = 0; p < Np; p++) {
    ...
    */
    ...
    /*
      Replace the deformation gradient at t = n with the converged deformation
      gradient
    */
    for (int i = 0; i < Ndim; i++) {
      for (int j = 0; j < Ndim; j++) {
      ...
      */
      for (int i = 0; i < Ndim; i++) {
        ...
        MPM_Mesh.Phi.acc.nM[p][i] +=
            alpha_1 * D_U_pI - alpha_2 * V_pI - (alpha_3 + 1) * A_pI;
        MPM_Mesh.Phi.vel.nM[p][i] +=
            alpha_4 * D_U_pI + (alpha_5 - 1) * V_pI + alpha_6 * A_pI;
...
static void output_selector(Particle MPM_Mesh, Mesh FEM_Mesh, Mask ActiveNodes,
                            Matrix Velocity, Matrix Acceleration,
                            Matrix D_Displacement, Matrix Forces,
                            Matrix Reactions, Matrix Residual,
  ...
  */
  for (int i = 0; i < Number_Out_nodal_path_csv; i++) {
    ...
    if (Out_nodal_path_csv[i].Out_csv_nodes_path_Acceleration) {
      path_nodes_analysis_csv__InOutFun__(
          Acceleration, FEM_Mesh.Coordinates, "Nodal_path_acceleration_csv",
          ActiveNodes, Out_nodal_path_csv[i], i, TimeStep, DeltaTimeStep);
    }
    ...
    if (Out_nodal_path_csv[i].Out_csv_nodes_path_Residual) {
      path_nodes_analysis_csv__InOutFun__(
          Residual, FEM_Mesh.Coordinates, "Nodal_path_residual_csv",
          ActiveNodes, Out_nodal_path_csv[i], i, TimeStep, DeltaTimeStep);
    }
  }
