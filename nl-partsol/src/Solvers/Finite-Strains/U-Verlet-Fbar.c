...
int U_Verlet_Fbar_Finite_Strains(
  ...
  */
  ...
  for (int TimeStep = InitialStep; TimeStep < NumTimeStep; TimeStep++) {
    ...
    if(STATUS == EXIT_FAILURE)
    {
      fint(stderr,""RED"Error in __gravity_NODES"RESET" \n");
      return EXIT_FAILURE;
    }

    STATUS = __d_displacement_NODES(D_Displacement, MPM_Mesh, FEM_Mesh,
                                                  ActiveNodes, Lumped_Mass);
    if(STATUS == EXIT_FAILURE)
    {
      fint(stderr,""RED"Error in __d_displacement_NODES"RESET" \n");
      return EXIT_FAILURE;
    }

    impose_Dirichlet_Boundary_Conditions(FEM_Mesh, D_Displacement, 
