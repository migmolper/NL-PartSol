...
Load *GramsBodyForces(char *Name_File, int NumBodyForces, int GPxElement,
  ...
  while (fgets(Line_GramsBodyForces, sizeof(Line_GramsBodyForces), Sim_dat) !=
         NULL) {

    /* Read the line with the space as separators */
    Num_words_line =
        parse(Parse_GramsBodyForces, Line_GramsBodyForces, " \n\t");
    if (Num_words_line < 0) {
    ...
    if ((Num_words_line > 0) &&
      ...
      if ((Num_words_line != 2) || (strcmp(Parse_Nodes[0], "Nodes") != 0)) {
        fprintf(stderr, "%s : %s \n", "Error in GramsBodyForces()",
      ...
      for (int i = 0; i < Num_Nodes; i++) {
        for (int j = 0; j < GPxElement; j++) {
          B[IndexLoad].Nodes[i * GPxElement + j] =
              Array_Nodes[i] * GPxElement + j;
        }
      }
      ...
      B[IndexLoad].Value = (Curve *)Allocate_Array(NumberDOF, sizeof(Curve));
        ...
        if ((Num_words_line > 0) && (strcmp(Parse_Properties[0], "}") != 0)) {
          if (strcmp(Parse_Properties[0], "b.x") == 0) {
            ...
            if (strcmp(Parse_Properties[1], "NULL") != 0) {
              ...
              active_direction(&B[IndexLoad].Dir[NumTimeStep * 0],
                               IMIN(NumTimeStep, B[IndexLoad].Value[0].Num));
              printf(" \t %s (%i) : \n \t %s %s \n", "* Body BC",
                     B[IndexLoad].NumNodes, Parse_Properties[1], FileLoadRoute);
            }
            ...
            if (strcmp(Parse_Properties[1], "NULL") != 0) {
              ...
              active_direction(&(B[IndexLoad].Dir[NumTimeStep * 1]),
                               IMIN(NumTimeStep, B[IndexLoad].Value[1].Num));
              printf(" \t %s (%i) : \n \t %s %s \n", "* Body BC",
                     B[IndexLoad].NumNodes, Parse_Properties[1], FileLoadRoute);
            }
            ...
            if (strcmp(Parse_Properties[1], "NULL") != 0) {
              ...
              active_direction(&B[IndexLoad].Dir[NumTimeStep * 2],
                               IMIN(NumTimeStep, B[IndexLoad].Value[2].Num));
              printf(" \t %s (%i) : \n \t %s %s \n", "* Body BC",
                     B[IndexLoad].NumNodes, Parse_Properties[1], FileLoadRoute);
            } else {
              B[IndexLoad].Dir[2] = 0;
