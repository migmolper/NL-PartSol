  ...
  if (Num_NCE > 0) {
    ...
    while (fgets(line, sizeof line, Sim_dat) != NULL) {
      ...
      if ((nkwords > 0) && (strcmp(kwords[0], "Out-particles-path-csv") == 0)) {
        ...
        Out_particles_path_csv[i_CSV] =
            fill_CSV_Parameters(CSV_Intervals, CSV_Parameters);
...
static void standard_error(char *Error_message) {
  fprintf(stderr, "%s : %s !!! \n", "Error in Out-particles-path-csv()",
          Error_message);
  exit(EXIT_FAILURE);
}
...
static Intervals read_CSV_Intervals(char *Interval_message) {
  ...
  if (CSV_Intervals.i_end < CSV_Intervals.i_start) {
    standard_error("The initial result time step should be less than the final "
                   "result time");
  }
  ...
  if (Is_Open && Is_Close) {
    if (Is_DIR && Is_PATH) {
      ...
      sprintf(Error_message, "%s",
              "You forgot to define something or close the statement");
      standard_error(Error_message);
    }
  } else {
...
static Event fill_CSV_Parameters(Intervals CSV_Intervals,
                                 Parameters CSV_Parameters) {
  Event CSV_Event;
  ...
  CSV_Event.Idx_Path =
      set_to_memory__SetLib__(Chain_Particles, CSV_Event.Lenght_Path);
