...
static void interpolate_initial_values_particles(Particle, Mesh, char *,
                                                 Tensor);

/***************************************************************************/

void Initial_condition_nodes__InOutFun__(char *Name_File, Particle MPM_Mesh,
                                         Mesh FEM_Mesh)
/*
...
*/
{
  ...
  while (fgets(line, sizeof line, Sim_dat) != NULL) {
    ...
    if ((nkwords > 0) && (strcmp(kwords[0], "Initial-nodal-values") == 0)) {
      ...
      interpolate_initial_values_particles(MPM_Mesh, FEM_Mesh, Route_NODES,
                                           Field);
...
static void Read_list_nodes_file(char *Route_NODES, char *List_nodes_string,
                                 char *Name_File) {
...
static Tensor Read_initial_values(FILE *Simulation_file) {
  ...
  while (fgets(Parameter_line, sizeof(Parameter_line), Simulation_file) !=
    ...
    if ((strcmp(Parameter_pars[0], "{") == 0) && (Parser_status == 1)) {
      Is_Open = true;
    } else if ((strcmp(Parameter_pars[0], "VELOCITY") == 0) &&
               (Parser_status == 2)) {
...
static Tensor Read_initial_vectorial_field(char *String_Values) {
  ...
  if (Parser_status == Ndim) {
    ...
    sprintf(Error_message, "%s %i-D %s %i-D %s", "You put a", Parser_status,
            "vector in a", Ndim, "problem");
    standard_error(Error_message);
  }
...
static void interpolate_initial_values_particles(Particle MPM_Mesh,
                                                 Mesh FEM_Mesh,
                                                 char *Route_NODES,
                                                 Tensor Field) {
