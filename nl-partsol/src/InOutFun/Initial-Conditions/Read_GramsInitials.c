...
void Initial_condition_particles__InOutFun__(char *Name_File, Particle GP_Mesh,
                                             int GPxElement)
/*
...
*/
{
  ...
  /* Initial message */
  puts("*************************************************");
  printf(" \t %s : \n\t %s \n", "* Read initial conditions in ", Name_File);
  ...
  while (fgets(Line_GramsInitials, sizeof(Line_GramsInitials), Sim_dat) !=
         NULL) {
    ...
    if ((Num_words_parse > 0) &&
      ...
      if (strcmp(Parse_GramsInitials[2], "{") == 0) {
        /* Initial line */
        STATUS_LINE = fgets(Line_Init_Prop, sizeof(Line_Init_Prop), Sim_dat);
        if (STATUS_LINE == NULL) {
          fprintf(stderr, "%s : %s \n", "Error in GramsInitials()",
                  "Unspected EOF !!!");
          exit(EXIT_FAILURE);
