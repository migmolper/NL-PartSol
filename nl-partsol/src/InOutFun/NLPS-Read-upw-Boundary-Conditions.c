...
static BCC_Properties Read_Boundary_Conditions_Properties(FILE *, char *);
...
*/
{
	...
	*/
	while(fgets(Line_GramsBoundary, sizeof(Line_GramsBoundary), Sim_dat) != NULL )
	{
		...
		*/
    	Num_words_line = parse (Parse_GramsBoundary, Line_GramsBoundary,delimiters_1);
    	...
    	if ((Num_words_line > 0) && (strcmp(Parse_GramsBoundary[0],"GramsBoundary") == 0 ))
    	{
    		...
    		*/
    		if( (Num_words_line != 2) || (strcmp(Parse_Nodes[0],"File") != 0))
      		{
				sprintf(Error_message,"Sintax error in line %i : GramsBoundary(File=Nodes.txt)",Num_line);
    			standard_error();
      		}
			...
			/* 
     		...
     		Bounds.BCC_i[IndexBoundary].Nodes = set_to_memory__SetLib__(Chain_Nodes,Bounds.BCC_i[IndexBoundary].NumNodes);
			...
			/*
				Read parameters
			*/
			BCC_Properties Parameters = Read_Boundary_Conditions_Properties(Sim_dat, Name_File);
...
static BCC_Properties Read_Boundary_Conditions_Properties(FILE * Simulation_file, char * Name_File)
{
	...
	while(fgets(Parameter_line, sizeof(Parameter_line), Simulation_file) != NULL)
  	{
    	...
    	*/    
    	...
    	else if((strcmp(Parameter_pars[0],"BcDirichlet") == 0) && (Parser_status == 3))
    	{
    		...
    		else if(strcmp(Parameter_pars[1],"V.z") == 0)
    		{
    			if(strcmp(Parameter_pars[2],"NULL") != 0)
    			{
	    			Properties.Dir[Ndim-1] = 1;
    				sprintf(FileLoadRoute,"%s%s",Route_Nodes,Parameter_pars[2]);
    				Properties.Value[Ndim-1] = ReadCurve(FileLoadRoute);
    			}
    		}
    		else if(strcmp(Parameter_pars[1],"P") == 0)
    		{
    			if(strcmp(Parameter_pars[2],"NULL") != 0)
    			{
    		...
    		{
	    	    sprintf(Error_message,"Sintax error in line %i : BcDirichlet ->%s<- %s",Num_line,Parameter_pars[2],Parameter_pars[3]);
    	    	standard_error(); 
    		}
    	}
    	...
    	{
			sprintf(Error_message,"Sintax error in line %i : Undefined parameter %s",Num_line,Parameter_pars[0]);
    	    standard_error();
    	}
