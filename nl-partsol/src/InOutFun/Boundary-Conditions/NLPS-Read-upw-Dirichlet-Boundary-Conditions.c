...
Boundaries Read_upw_Dirichlet_Boundary_Conditions__InOutFun__(char *Name_File,
                                                              int NumBounds,
                                                              int NumTimeStep)
/*
...
*/
      ...
      */
      strcpy(Bounds.BCC_i[IndexBoundary].Info,
             "Displacements-Pore-Water-Pressure");
    ...
    if ((strcmp(Parameter_pars[0], "{") == 0) && (Parser_status == 1)) {
      ...
      if (strcmp(Parameter_pars[1], "V.x") == 0) {
        ...
        if (strcmp(Parameter_pars[2], "NULL") != 0) {
          ...
          active_direction(&Properties.Dir[NumTimeStep * (Ndim - 1)],
                           IMIN(NumTimeStep, Properties.Value[Ndim - 1].Num));
          printf("\t \t %s (%s) : \n \t \t Number of nodes = %i \n \t \t File "
                 "curve %s \n",
                 "-> BcDirichlet ", Parameter_pars[1], NumNodes, FileLoadRoute);
        }
        ...
        if (strcmp(Parameter_pars[2], "NULL") != 0) {
          ...
          active_direction(&Properties.Dir[NumTimeStep * Ndim],
                           IMIN(NumTimeStep, Properties.Value[Ndim].Num));
          printf(" \t %s (%s) : \n \t \t Number of nodes = %i \n \t \t File "
                 "curve %s \n",
                 "-> BcDirichlet ", Parameter_pars[1], NumNodes, FileLoadRoute);
        }
