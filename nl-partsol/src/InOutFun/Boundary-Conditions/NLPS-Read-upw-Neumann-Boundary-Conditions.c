...
*/
{
	...
	*/
	while(fgets(Line_GramsBoundary, sizeof(Line_GramsBoundary), Sim_dat) != NULL )
	{
    	...
    	*/
    	if ((Num_words_line > 0) && (strcmp(Parse_GramsBoundary[0],"Define-Neumann-Boundary") == 0 ))
    	{
    		...
    		*/
    		if( (Num_words_line != 2) || (strcmp(Parse_Nodes[0],"File") != 0))
      		{
				sprintf(Error_message,"Sintax error in line %i : Define-Neumann-Boundary(File=Nodes.txt)",Num_line);
    			standard_error();
      		}
			...
			/* 
     		...
     		Bounds.BCC_i[IndexBoundary].Nodes = (int *)Allocate_ArrayZ(GPxElement*NumElements,sizeof(int));
     		for(int i = 0 ; i<NumElements ; i++)
     		{
     			...
     			{
     				Bounds.BCC_i[IndexBoundary].Nodes[i*GPxElement+j] = Element_idx*GPxElement+j;
     			}
	...
	while(fgets(Parameter_line, sizeof(Parameter_line), Simulation_file) != NULL)
  	{
    	...
    	*/    
    	...
    	else if((Parser_status == 2) && (strcmp(Parameter_pars[0],"T.x") == 0))
    	{
    	...
    	else if((Parser_status == 2) && (strcmp(Parameter_pars[0],"T.y") == 0))
    	{
    		if(strcmp(Parameter_pars[1],"NULL") != 0)
    		{
	    		Properties.Dir[1] = 1;
    	...
    	else if((Parser_status == 2) && (strcmp(Parameter_pars[0],"T.z") == 0))
    	{
    		if(strcmp(Parameter_pars[1],"NULL") != 0)
    		{
