...
Particle Generate_Gauss_Point_Analysis__InOutFun__(char *SimulationFile) {
  ...
  PointAnalysis.Mat =
      Read_Materials__InOutFun__(SimulationFile, NumberMaterials);

  PointAnalysis.Phi = allocate_U_vars__Fields__(NumTimeStep);

  Strain_curve *Strain_Case =
      Read_Strains_curves(PointAnalysis.Phi, SimulationFile, NumStrainCurves);
...
static Strain_curve *Read_Strains_curves(Fields Phi, char *SimulationFile,
                                         int NumStrainCurves)
/*
...
*/
{
  ...
  Strain_curve *Strain_Case =
      (Strain_curve *)malloc(NumStrainCurves * sizeof(Strain_curve));
...
static int Number_Output_directives(char *Name_File) {
  ...
  while (fgets(line, sizeof line, Sim_dat) != NULL) {
    ...
    if ((nkwords > 0) &&
        (strcmp(kwords[0], "Particle-variables-evolution-to-csv") == 0)) {
      NOD++;
    }
  }
...
*/
{
  ...
  while (fgets(line, sizeof line, Sim_dat) != NULL) {
    ...
    if ((nkwords > 0) &&
      ...
      Out_Gauss_Point_evolution_csv[idx] =
          Fill_CSV_Output_Directives(CSV_Intervals, CSV_Parameters);
...
static Intervals Read_CSV_Intervals(char *Interval_message) {
  ...
  for (int i = 0; i < Interval_status_1; i++) {
    ...
    else if ((strcmp(Aux_Parse_2[0], "i_step") == 0) &&
             (Interval_status_2 == 2)) {
      CSV_Intervals.i_step = atoi(Aux_Parse_2[1]);
    }

    else if ((strcmp(Aux_Parse_2[0], "i_end") == 0) &&
             (Interval_status_2 == 2)) {
      CSV_Intervals.i_end = atoi(Aux_Parse_2[1]);
    } else {
      sprintf(Error_message, "The statement %s is not recognised",
              Aux_Parse_2[0]);
      standard_error();
    }
  }
  ...
  if (CSV_Intervals.i_end < CSV_Intervals.i_step) {
    sprintf(Error_message,
            "The result interval step should be less than final time");
    standard_error();
  }
  if (CSV_Intervals.i_end < CSV_Intervals.i_start) {
    sprintf(Error_message, "The initial result time step should be less than "
                           "the final result time");
    standard_error();
  }
  if (CSV_Intervals.i_end > NumTimeStep) {
    sprintf(Error_message,
            "The final result time step should be less than the final time");
    standard_error();
  }
...
static Parameters Read_CSV_Parameters(FILE *Simulation_file) {
  ...
  while (fgets(Line_Out_Prop, sizeof(Line_Out_Prop), Simulation_file) != NULL) {
    ...
    if ((strcmp(Parse_Out_Prop[0], "{") == 0) && (Aux_Out_id == 1)) {
      Is_Open = true;
    } else if ((strcmp(Parse_Out_Prop[0], "Directory") == 0) &&
               (Aux_Out_id == 2)) {
      ...
      Output_csv.Is_Defined_directory =
          Check_Output_directory(Output_csv.Directory_Name);
...
static Event Fill_CSV_Output_Directives(Intervals CSV_Intervals,
                                        Parameters CSV_Parameters) {
  Event CSV_Event;
...
static bool Check_Output_directory(char *Output_directory) {
  ...
  if (S_ISDIR(info.st_mode)) {
    ...
    sprintf(Error_message, "%s : %s %s \n", "Output directory",
            Output_directory, "does not exists");
    standard_error();
  }
...
static void standard_error() {
  fprintf(stderr, "%s : %s !!! \n", "Error in Generate_Gauss_Point_Analysis()",
          Error_message);
  exit(EXIT_FAILURE);
}
