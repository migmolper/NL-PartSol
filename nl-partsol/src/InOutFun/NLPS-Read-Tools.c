...
int Parse_string__InOutFun__(char ** words, char * str, char * delims, int * STATUS)
{
	char * delims_cpy = (char *)malloc((strlen(delims))*sizeof(char));
  	...
  	for (p = strtok (str, delims_cpy); p; p = strtok (NULL, delims_cpy))
  	{
    	...
    	{ 
			...
			"Parse_string__InOutFun__","MAXW reached");
			(*STATUS) = 1;
	  		return n;
		}
	}
    ...
    { 
		fprintf(stderr,"Error in : %s. %s \n",
			"Parse_string__InOutFun__","Parser failed");
		(*STATUS) = 1;
	}
...
Tensor Read_Vector__InOutFun__(
	char * String_vector,
	int * STATUS)
/*!
 * Vector = {x, y}
