...
*/
{
  ...
  while(fgets(line, sizeof line, Sim_dat) != NULL)
  {
        ...
        {
          List_Materials[idx] = Define_Solid_Rigid(Sim_dat,Index_and_Model.Model,Index_and_Model.Idx);
        }
        else if(strcmp(Index_and_Model.Model,"LE") == 0)
        {
          List_Materials[idx] = Define_Linear_Elastic(Sim_dat,Index_and_Model.Model,Index_and_Model.Idx);
        }
        else if(strcmp(Index_and_Model.Model,"Saint-Venant-Kirchhoff") == 0)
        { 
          List_Materials[idx] = Define_Saint_Venant_Kirchhoff(Sim_dat,Index_and_Model.Model,Index_and_Model.Idx);
        }
        else if(strcmp(Index_and_Model.Model,"Neo-Hookean-Wriggers") == 0)
        {
          List_Materials[idx] = Define_Neo_Hookean_Wriggers(Sim_dat,Index_and_Model.Model,Index_and_Model.Idx);
        }
        else if(strcmp(Index_and_Model.Model,"Von-Mises") == 0)
        { 
          List_Materials[idx] = Define_Von_Mises(Sim_dat,Index_and_Model.Model,Index_and_Model.Idx);
        }
