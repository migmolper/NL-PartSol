...
*/
{
  ...
  while (fgets(line, sizeof line, Sim_dat) != NULL) {
    ...
    if ((nkwords > 0) && (strcmp(kwords[0], "GramsMaterials") == 0)) {
      ...
      if (strcmp(kwords[2], "{") == 0) {
        ...
        /* Initial line */
        STATUS_LINE =
            fgets(Line_Material_Prop, sizeof(Line_Material_Prop), Sim_dat);
        if (STATUS_LINE == NULL) {
        ...
        while (STATUS_LINE != NULL) {
          ...
          /**************************************************/
            ...
            if (strcmp(Parse_Mat_Prop[1], "true") == 0) {
              ...
              sprintf(Error_message,
                      "The input was %s. Please, use : true/false",
                      Parse_Mat_Prop[1]);
              standard_error(Error_message);
            }
          }
          ...
          /**************************************************/
            ...
            if (strcmp(Parse_Mat_Prop[1], "Hughes") == 0) {
              ...
              sprintf(Error_message, "%s",
                      "Options for Hardening-Criteria -> "
                      "Hughes/Cervera/Ortiz/Voce");
              standard_error(Error_message);
            }
          }
          /**************************************************/
          ...
          else if (strcmp(Parse_Mat_Prop[0], "Parameter-Hardening-Hughes") ==
                   0) {
          ...
          /**************************************************/
            ...
            rad_friction_angle =
                (PI__MatrixLib__ / 180) * Mat_GP.phi_Frictional;
          }
          /**************************************************/
            ...
            rad_dilatancy_angle =
                (PI__MatrixLib__ / 180) * Mat_GP.psi_Frictional;
          }
          /**************************************************/
          ...
          /**************************************************/
            ...
            sprintf(Error_message, "%s %s %s", "the propertie",
                    Parse_Mat_Prop[0], "is not defined");
            standard_error(Error_message);
          }
  ...
  if (CountMaterials != GP_Mesh.NumberMaterials) {
    sprintf(Error_message, "%s %i %s %i %s \n", "Spected",
            GP_Mesh.NumberMaterials, "materials, but", CountMaterials,
            "where defined");
    standard_error(Error_message);
  }
...
static void check_Newtonian_Fluid_Compressible_Material(Material Mat_particle) {
  ...
  if (Is_rho && Is_Cel && Is_Compressibility && Is_ReferencePressure &&
      Is_Viscosity && Is_n_Macdonald_model) {
    printf("\t -> %s \n", "Newtonian-Compressible fluid");
    printf("\t \t -> %s : %f \n", "Celerity", Mat_particle.Cel);
    printf("\t \t -> %s : %f \n", "Density", Mat_particle.rho);
    printf("\t \t -> %s : %f \n", "Compressibility",
           Mat_particle.Compressibility);
    printf("\t \t -> %s : %f \n", "Reference Pressure",
           Mat_particle.ReferencePressure);
    printf("\t \t -> %s : %f \n", "Viscosity", Mat_particle.Viscosity);
    printf("\t \t -> %s : %f \n", "Macdonald-parameter",
           Mat_particle.n_Macdonald_model);
  } else {
    fprintf(stderr, "%s : %s \n", "Error in GramsMaterials()",
      ...
      if (Mat_particle.Hardening_Hughes) {
        if (Is_Hardening_modulus && Is_Parameter_Hardening_Hughes) {
          ...
          fputs(Is_Hardening_modulus ? "Hardening-Modulus : true \n"
                ...
                stdout);
          fputs(Is_Parameter_Hardening_Hughes
        ...
        if (Is_Exponent_Hardening_Ortiz && Is_Reference_Plastic_Strain_Ortiz) {
          ...
          fprintf(stderr, "%s : %s \n", "Error in GramsMaterials()",
                  ...
                  "Hardening)");
          fputs(Is_Exponent_Hardening_Ortiz
        ...
        if (Is_theta_Hardening_Voce && Is_delta_Hardening_Voce &&
            Is_K_inf_Hardening_Voce && Is_K_0_Hardening_Voce) {
          printf("\t \t -> %s : %f \n", "K-0-Hardening-Voce",
                 Mat_particle.K_0_Hardening_Voce);
          printf("\t \t -> %s : %f \n", "K-inf-Hardening-Voce",
                 Mat_particle.K_inf_Hardening_Voce);
          printf("\t \t -> %s : %f \n", "delta-Hardening-Voce",
                 Mat_particle.delta_Hardening_Voce);
          printf("\t \t -> %s : %f \n", "theta-Hardening-Voce",
                 Mat_particle.theta_Hardening_Voce);
        } else {
          fprintf(stderr, "%s : %s \n", "Error in GramsMaterials()",
                  "Some parameter is missed for Von-Mises material (Voce "
                  "Hardening)");
