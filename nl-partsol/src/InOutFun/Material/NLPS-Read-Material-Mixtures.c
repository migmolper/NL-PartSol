...
Mixture *Read_Soil_Water_Mixtures__InOutFun__(char *SimulationFile,
...
*/
  Mixture *List_Mixtures =
      (Mixture *)malloc(Number_Soil_Water_Mixtures * sizeof(Mixture));
...
static Mixture Define_Mixture(FILE *Simulation_file,
  ...
  */
  ...
  while (fgets(Parameter_line, sizeof(Parameter_line), Simulation_file) !=
    ...
    } else if (strcmp(Parameter_pars[0], "Reference-Volume-Fraction-Soil") ==
               0) {
      ChkMix.Is_phi_s_0 = true;
      New_Mixture.phi_s_0 = atof(Parameter_pars[1]);
    } else if (strcmp(Parameter_pars[0], "Reference-Volume-Fraction-Fluid") ==
               0) {
...
static void check_Borja_2004_Soil_water_Mixture(Mixture Mix_particle,
                                                Check_Mixture ChkMix) {

  if (ChkMix.Is_Soil_Idx && ChkMix.Is_Water_Idx && ChkMix.Is_Permeability &&
      ChkMix.Is_phi_s_0 && ChkMix.Is_phi_f_0) {
    printf("\t -> %s \n", "Soil-Water mixture (Borja 2004)");
    printf("\t \t -> %s : %i \n", "Material model for the Soil phase",
           Mix_particle.Soil_Idx);
    printf("\t \t -> %s : %i \n", "Material model for the Fluid phase",
           Mix_particle.Water_Idx);
    printf("\t \t -> %s : %f \n", "Volume fraction (Soil phase)",
           Mix_particle.phi_s_0);
    printf("\t \t -> %s : %f \n", "Volume fraction (fluid phase)",
           Mix_particle.phi_f_0);
  } else {
    fprintf(stderr, "%s : %s %s %s\n", "Error in Define-Mixture()",
            "Some parameter is missed for", Mix_particle.Type, "Mixture");
