...
int STATUS = EXIT_SUCCESS;
  ...
  if (ChkMat.Is_rho && ChkMat.Is_E && ChkMat.Is_nu && ChkMat.Is_Reference_pressure &&
    ...
    printf("\t \t -> %s : %f \n", ""MAGENTA"Density"RESET"", (*DP_Material).rho);

    printf("\t \t -> %s : %f \n", ""MAGENTA"Elastic modulus"RESET"", (*DP_Material).E);

    printf("\t \t -> %s : %f \n", ""MAGENTA"Poisson modulus"RESET"", (*DP_Material).nu);

    printf("\t \t -> %s : %f \n", ""MAGENTA"Friction-angle"RESET"",
           (*DP_Material).phi_Frictional * (180 / PI__MatrixLib__));

    printf("\t \t -> %s : %f \n", ""MAGENTA"Dilatancy-angle"RESET"",
           (*DP_Material).psi_Frictional * (180 / PI__MatrixLib__));


    if (ChkMat.Is_kappa_0 && ChkMat.Is_Hardening_Exponent && ChkMat.Is_Hardening_modulus && ChkMat.Is_J2_degradated) {
