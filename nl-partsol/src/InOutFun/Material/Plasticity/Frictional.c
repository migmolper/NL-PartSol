...
Material Define_Frictional(FILE *Simulation_file, char *Material_Model,
                           int Material_Idx) {
  ...
  while (fgets(Parameter_line, sizeof(Parameter_line), Simulation_file) !=
    ...
    /**************************************************/
      ...
      if ((strcmp(Parameter_pars[1], "Matsuoka-Nakai") == 0) ||
        ...
        strcpy(Frictional_Material.Yield_Function_Frictional,
               Parameter_pars[1]);
        ChkMat.Is_Yield_Function_Frictional = true;
      } else {
        fprintf(stderr, "%s : %s \n", "Error in GramsMaterials()",
                "Unrecognized yield function");
        exit(EXIT_FAILURE);
      }
    ...
    /**************************************************/
      ...
      Frictional_Material.phi_Frictional =
          (PI__MatrixLib__ / 180) * atof(Parameter_pars[1]);
    }
    /**************************************************/
    else if (strcmp(Parameter_pars[0], "Dilatancy-angle") == 0) {
      ChkMat.Is_dilatancy_angle = true;
      Frictional_Material.psi_Frictional =
          (PI__MatrixLib__ / 180) * atof(Parameter_pars[1]);
    }
    /**************************************************/
    else if (strcmp(Parameter_pars[0], "Fbar") == 0) {
      ChkMat.Is_Locking_Control_Fbar = true;
      Frictional_Material.Locking_Control_Fbar =
          Activate_Options(Parameter_pars[1]);
    }
    /**************************************************/
...
static void check_Frictional_Material(Material Mat_particle,
                                      Check_Material ChkMat, int Idx) {
  if (ChkMat.Is_rho && ChkMat.Is_E && ChkMat.Is_nu &&
      ChkMat.Is_Yield_Function_Frictional && ChkMat.Is_atmospheric_pressure &&
      ChkMat.Is_alpha_Hardening_Borja && ChkMat.Is_a1_Hardening_Borja &&
    ...
    printf("\t \t -> %s : %s \n", "Yield function",
           Mat_particle.Yield_Function_Frictional);
    printf("\t \t -> %s : %f \n", "Atmospheric-pressure",
           Mat_particle.atmospheric_pressure);
    printf("\t \t -> %s : %f \n", "alpha-Hardening-Borja",
           Mat_particle.alpha_Hardening_Borja);
    printf("\t \t -> %s : %f \n", "a1-Hardening-Borja",
           Mat_particle.a_Hardening_Borja[0]);
    printf("\t \t -> %s : %f \n", "a2-Hardening-Borja",
           Mat_particle.a_Hardening_Borja[1]);
    printf("\t \t -> %s : %f \n", "a3-Hardening-Borja",
           Mat_particle.a_Hardening_Borja[2]);
    printf("\t \t -> %s : %f \n", "Friction-angle",
           Mat_particle.phi_Frictional * (180 / PI__MatrixLib__));
    printf("\t \t -> %s : %f \n", "Dilatancy-angle",
           Mat_particle.psi_Frictional * (180 / PI__MatrixLib__));
...
void Initialize_Frictional(double *kappa, double *EPS, Material MatProp) {
    ...
    *EPS -= f / df;
    ...
    if (iter > 10) {
      sprintf(Error_message,
              "Failure during the initialization of the Matsuoka-Nakai \n");
      standard_error();
    }
  }
}
