  	...
  	while(fgets(line, sizeof line, Data_File) != NULL )
  	{
    	...
    	if (strcmp(kwords[0],"#") == 0)
    	{
    		for(int i = 1 ; i<nkwords ; i++)
    		{
    			...
    			{
    				Parameters.Parser_string = Read_PARSER_param(parameters[1],Parameters.N_Cols);
    				Is_PARSER = true;
    			}
...
static Matrix Read_data(Param_Data Header_Parameters, FILE * Data_File)
{
	...
	char * STATUS_line = {NULL};
  	...
  	for(int i = 0 ; i<N_Rows ; i++)
  	{
		...
		if(nkwords != N_Cols)
 		{
			sprintf(Error_message,"%s %i %s","The line",i+1,"does not match with the parser");
			standard_error();	
  		}
...
static void Check_header_parameters(bool Is_NROWS, bool Is_NCOLS, bool Is_PARSER)
{
