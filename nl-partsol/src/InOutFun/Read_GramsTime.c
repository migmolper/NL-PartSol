...
Time_Int_Params Solver_selector__InOutFun__(char *Name_File) {
  ...
  while (fgets(line, sizeof line, Sim_dat) != NULL) {
    ...
    if ((nkwords > 0) && (strcmp(kwords[0], "NLPS-Solver") == 0)) {
      ...
      if ((Aux_Temp_id != 2) || (strcmp(Parse_Temp_id[0], "Type") != 0)) {
      ...
      if (strcmp(kwords[2], "{") == 0) {
        ...
        while (STATUS_LINE != NULL) {
          ...
          if (strcmp(Parse_Temp_Prop[0], "CFL") == 0) {
            ...
            printf("\t \t -> %s : %f \n", "Epsilon",
                   Parameters.epsilon_Mass_Matrix);
            ...
            printf("\t \t -> %s : %f \n", "Tolerance Newmark-beta",
                   Parameters.TOL_Newmark_beta);
        ...
        fprintf(stderr, "%s : %s \n", "Error in NLPS-Solver()",
                "Use this format -> NLPS-Solver (Type=string) { !!!");
        exit(EXIT_FAILURE);
      }
    }
  }
...
static void check_Solver(Time_Int_Params Parameters, Check_Params ChkParam) {

  if (ChkParam.Is_Solver == false) {
    fprintf(stderr, "%s : %s \n", "Error in NLPS-Solver()",
            "NLPS-Solver no defined");
    exit(EXIT_FAILURE);
  }

  if (ChkParam.Counter_Solver != 1) {
    fprintf(stderr, "%s : %s \n", "Error in NLPS-Solver()",
            "More than one call to NLPS-Solver");
    exit(EXIT_FAILURE);
  }

  if (ChkParam.Is_NumTimeStep && ChkParam.Is_Cel && ChkParam.Is_CFL) {
    printf("\t \t -> %s : %i \n", "Number of TimeStep", Parameters.NumTimeStep);
    printf("\t \t -> %s : %f \n", "Celerity", Parameters.Cel);
    printf("\t \t -> %s : %f \n", "Courant condition", Parameters.CFL);

    if (strcmp(Parameters.TimeIntegrationScheme,
               "Newmark-beta-Finite-Strains") == 0) {
      if (ChkParam.Is_beta_Newmark_beta && ChkParam.Is_gamma_Newmark_beta &&
          ChkParam.Is_TOL_Newmark_beta && ChkParam.Is_Epsilon_Mass_Matrix) {
        printf("\t \t -> %s : %s \n", "Time integrator",
               Parameters.TimeIntegrationScheme);
        printf("\t \t \t -> %s : %f \n", "beta", Parameters.beta_Newmark_beta);
        printf("\t \t \t -> %s : %f \n", "gamma",
               Parameters.gamma_Newmark_beta);
        printf("\t \t \t -> %s : %e \n", "epsilon",
               Parameters.epsilon_Mass_Matrix);
        printf("\t \t \t -> %s : %e \n", "TOL", Parameters.TOL_Newmark_beta);
      } else {
        fprintf(stderr, "%s : %s \n", "Error in NLPS-Solver()",
                "Some parameter for the Newmark-beta-Finite-Strains");
    ...
    } else if (strcmp(Parameters.TimeIntegrationScheme, "Generalized-alpha") ==
               0) {
      if (ChkParam.Is_rb_Generalized_alpha &&
        ...
        printf("\t \t -> %s : %s \n", "Time integrator",
               Parameters.TimeIntegrationScheme);
        printf("\t \t \t -> %s : %f \n", "radius",
               Parameters.rb_Generalized_alpha);
        printf("\t \t \t -> %s : %e \n", "TOL",
               Parameters.TOL_Generalized_alpha);
      } else {
        fprintf(stderr, "%s : %s \n", "Error in NLPS-Solver()",
                "Some parameter for the Generalized-alpha");
    ...
    } else if (strcmp(Parameters.TimeIntegrationScheme,
      ...
      if (ChkParam.Is_rb_Generalized_alpha &&
        ...
        printf("\t \t \t -> %s : %e \n", "epsilon",
               Parameters.epsilon_Mass_Matrix);
        printf("\t \t \t -> %s : %e \n", "TOL",
               Parameters.TOL_Conserving_Energy_Momentum);
      } else {
        fprintf(stderr, "%s : %s \n", "Error in NLPS-Solver()",
                "Some parameter for the Discrete-Energy-Momentum");
        fputs(ChkParam.Is_rb_Generalized_alpha ? "radius : true \n"
                                               : "radius : false \n",
              stdout);
        fputs(ChkParam.Is_Epsilon_Mass_Matrix ? "epsilon : true \n"
