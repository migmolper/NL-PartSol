...
bool Out_elastic_deformation_gradient = false;
...
*/
{
  ...
  while( fgets(line, sizeof line, Sim_dat) != NULL ){
	...
	while(STATUS_LINE != NULL){
	  ...
	  else if(strcmp(Parse_Out_Prop[0],"Out-elastic-deformation-gradient") == 0)
	  {
	  	Out_elastic_deformation_gradient = Is_Output_Activate(Parse_Out_Prop[0],Parse_Out_Prop[1]);
	  }
