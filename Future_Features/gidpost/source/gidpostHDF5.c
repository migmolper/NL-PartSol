...
void initMeshGroupData();
const char *getCurrentMeshPath();
const char *getCurrentResultPath();
...
int GiD_BeginMesh_HDF5(GP_CONST char * MeshName,GiD_Dimension Dim, GiD_ElementType EType,int NNode)
{
  ...
  const char *mesh_path = getCurrentMeshPath();
  hdf5c_create_group( mesh_path);
  current_mesh_num++;
  sprintf(meshN,"%s/%d", mesh_path, current_mesh_num);
...
int GiD_MeshUnit_HDF5(char * UnitName)
{
  char meshN[1024];
  if(UnitName){
    sprintf(meshN,"%s/%d", getCurrentMeshPath(), current_mesh_num);
...
{
  char setN[1024];
  sprintf(setN,"%s/%d/Coordinates", getCurrentMeshPath(), current_mesh_num);
...
int GiD_BeginElements_HDF5()
  ...
  sprintf(setN,"%s/%d/Elements", getCurrentMeshPath(), current_mesh_num);
...
{  
  ...
  const char *results_path = getCurrentResultPath();
  sprintf(resN,"%s/%d", results_path, num_results_total);
  strcpy(myresults[num_results_group-1].name,resN);

  hdf5c_create_group( results_path);
...
int GiD_Write2DComplexMatrix_HDF5(int id,
...
int GiD_WriteComplexMatrix_HDF5(int id,
...
/* MeshGroups: */
static int G_current_mesh_group_id = 0, G_using_mesh_group = 0;
static int G_current_on_mesh_group_id = 0, G_using_on_mesh_group = 0;
static int G_num_mesh_groups = 0;
static char **G_lst_mesh_groups_names = NULL;

int AddMeshGroupName( const char *name) {
  if ( !name) return -1;
  int ret_idx = G_num_mesh_groups;
  /* may be we should check for repetitions ... */
  G_lst_mesh_groups_names = realloc( G_lst_mesh_groups_names, ( G_num_mesh_groups + 1) * sizeof( char *));
  G_lst_mesh_groups_names[ ret_idx] = strdup( name);
  G_num_mesh_groups++;
  return ret_idx ;
}

int GetMeshGroupNameIdx( const char *name) {
  if ( !name) return -1;
  int ret_idx = -1;
  int i = 0;
  for ( i = 0; i < G_num_mesh_groups; i++) {
    if ( !strcmp( G_lst_mesh_groups_names[ i], name)) {
      ret_idx = i;
      break;
    }
  }
...
void initMeshGroupData() {
  G_current_mesh_group_id = 0;
  G_using_mesh_group = 0;
  G_current_on_mesh_group_id = 0;
  G_using_on_mesh_group = 0;
  int i = 0;
  for ( i = 0; i < G_num_mesh_groups; i++) {
    if ( G_lst_mesh_groups_names && G_lst_mesh_groups_names[ i]) {
      free( G_lst_mesh_groups_names[ i]);
    }
  }
  free( G_lst_mesh_groups_names);
  G_lst_mesh_groups_names = NULL;
  G_num_mesh_groups = 0;
}

static char G_current_mesh_group_name[ 2048];

int GiD_BeginMeshGroup_HDF5( const char *Name) {
  G_current_mesh_group_id = AddMeshGroupName( Name);
  strcpy( G_current_mesh_group_name, Name);
  G_using_mesh_group = 1;
  return 0;
}

int GiD_EndMeshGroup_HDF5() {
  G_using_mesh_group = 0;
  return 0;
}

static char G_current_on_mesh_group_name[ 2048];
int GiD_BeginOnMeshGroup_HDF5( const char *Name) {
  G_current_on_mesh_group_id = GetMeshGroupNameIdx( Name);
  if ( G_current_on_mesh_group_id == -1)
    return -1;
  strcpy( G_current_mesh_group_name, Name);
  G_using_on_mesh_group = 1;
  return 0;
}

int GiD_EndOnMeshGroup_HDF5() {
  G_using_on_mesh_group = 0;
  return 0;
}

static char G_current_mesh_group_path[ 2048];
const char *getCurrentMeshGroupPath() {
  const char *ret = NULL;
  if ( G_using_mesh_group && ( G_current_mesh_group_id >= 0)) {
    sprintf( G_current_mesh_group_path, "MeshGroup%d", G_current_mesh_group_id + 1);
    ret = G_current_mesh_group_path;
  }
  return ret;
}

static char G_mesh_path[ 2048];
const char *getCurrentMeshPath() {
  const char *mesh_group = getCurrentMeshGroupPath();
  if ( mesh_group) {
    hdf5c_create_group( mesh_group);
    hdf5c_set_attribute( mesh_group, "Name", G_current_mesh_group_name);
    sprintf( G_mesh_path, "%s/Meshes", mesh_group);
  } else {
    strcpy( G_mesh_path, "Meshes");
  }
  return G_mesh_path;
}

static char G_current_on_mesh_group_path[ 2048];
const char *getCurrentOnMeshGroupPath() {
  const char *ret = NULL;
  if ( G_using_on_mesh_group && ( G_current_on_mesh_group_id >= 0)) {
    sprintf( G_current_on_mesh_group_path, "MeshGroup%d", G_current_on_mesh_group_id + 1);
    ret = G_current_on_mesh_group_path;
  }
  return ret;
}

static char G_result_path[ 2048];
const char *getCurrentResultPath() {
  const char *result_group = getCurrentOnMeshGroupPath();
  if ( result_group) {
    hdf5c_create_group( result_group);
    hdf5c_set_attribute( result_group, "Name", G_current_mesh_group_name);
    sprintf( G_result_path, "%s/Results", result_group);
  } else {
    strcpy( G_result_path, "Results");
  }
  return G_result_path;
}
