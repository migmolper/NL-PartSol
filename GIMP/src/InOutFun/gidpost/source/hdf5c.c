...
int hdf5c_get_children_name(const char* group_path,int index,char* name,int max_len)
{
  ...
  len=H5Gget_objname_by_idx(global_group_id, ( hsize_t)index,&name[len0],( size_t)max_len-( size_t)len0);
  return ( int)( len+len0);
}
...
{  
  ...
  len0= ( ssize_t)strlen(name);
  len=H5Gget_objname_by_idx(group_id, ( hsize_t)index,&name[len0],( size_t)max_len-( size_t)len0);
  H5Gclose(group_id);
  return ( int)( len+len0);
}
...
int hdf5c_open_dataset_list(const char* obj_name,int* num_int,int* num_real)
{
  ...
  for(i=0;i< ( int)num;i++){
	...
	return -1;
      ...
      sprintf(errorBuffer,"array name '%s' cannot be retrieved. unknown type=%d,%llu",
