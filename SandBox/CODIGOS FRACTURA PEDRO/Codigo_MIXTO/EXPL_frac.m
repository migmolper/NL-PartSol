...
function EXPL_frac(xg)
    ...
    React2,Impact2,L_beam,L_hammer]=init_frac;
    ...
    [K_beam(1),K_hammer(1)]=initial_K(near,p,volume,dens,v(:,1),Material,sp); %Initial kinetic energy
    ...
    for ste=ste+1:step_final
        ...
        while C1 || C2
            ...
            if MASTER
                [as,C1]=contact(x_a1,lmass,vs,as);
            else
                C1=0;
            end
            ...
            C2=0;
            for j=1:SUP
                [as,Cc]=PS_surface(x_a1,lmass,vs,as,h,j);
                if Cc==1
                    C2=1;
                end
            end
...
%         end
        ...
        % 3. Residual    
        [~,int_Fs,status,d_energy,ss,es,FAIL]=Constitutive_frac(3,def_G,...
            near,volume,B,jacobians,h,ss,es,status,epsilon,d_energy,FAIL);
        ...
        if rem(ste,SAVE_I)==0;
            ...
            [W_beam(ste_p),W_hammer(ste_p),K_beam(ste_p),K_hammer(ste_p)]...
                = compute_energy(def_G,jacobians,volume,status,...
                near,p,dens,vs);
        ...
        if ((rem(ste/SAVE_I,SAVE_F)==0) || (ste==step_final) || (FAIL==1))
            save FILE elem x_0 x_a xg Area ste d v a Ps t tp Es Status ...
                ...
                React2 Impact2 L_beam L_hammer NO
        end
