...
function EXPL_frac_2(xg)
    ...
    global F1 H1
    h1=H1*0.05;
    Cr(1,:)=[F1 0 h1 inf];  % Initial notch
    [NO]=crack(xg,x_a,NO,Cr,1);
    for i=1:8
        Cr(i+1,:)=[F1 i*h1 h1 inf];  % Initial notch
    ...
    for ste=ste+1:step_final
        ...
        while C1 || C2
            ...
            if MASTER
                [as1,C1]=contact(x_a1,lmass,vs,as);
            else
                as1=as*0;
                C1=0;
            end
            ...
            if iter==1
                as1=as;
                for j=1:SUP
                    [as1,Cc]=PS_surface(x_a1,lmass,vs,as1,h,j);
                    if Cc
                        C2=1;
                    end
                end
...
%         end
        ...
        % 3. Residual    
        [~,int_Fs,status,d_energy,ss,es,FAIL,Wlist]=Constitutive_frac_2(3,def_G,...
