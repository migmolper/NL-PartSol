function [stiff_mtx,int_Fs,status,D_energy,ss,es,FAIL,Wlist,E_ini]=...
    Constitutive_ft(Kt,def_G,near,volume,B,jacobians,h,ss,es,status,...
    epsilon,D_energy,E_ini,FAIL)

    global nodes sp elements MODEL MAT MAT_el FRAC
    ...
    Max_e=zeros(elements,1);
    Crit=zeros(elements,1);
    FT=zeros(elements,1);
    Cauchy(elements,1)={0};
    Strain(elements,1)={0};
    EigenV(elements,1)={0};
            ...
            if  AA(1)==0
                ...
                elseif FRAC==3
                    ...
                    if status(e,2)==0  &&  E_ini(e)==0
                        ...
                        if eps_list(1)>0    
                            ...
                            if Crit(e)>0
                                ...
                                for i=1:length(eps_list)
                                    if Crit(eps_list(i))>0
                                        f=f+FT(eps_list(i))*volume(eps_list(i));
                                        M=M+volume(eps_list(i));
                                    else
                                        M=M+volume(eps_list(i));
                                    end
                    ...
                    elseif status(e,2)~=1
                        ...
                        chi=(Max_e(e)-E_ini(e))*30/wc;
                        status(e,1)=min(1,max(chi,status(e,2)));
                        T=(1-status(e,1))*T;
                        dd=Wlist(e)*volume(e);
                        eps_list=epsilon{e};
                        for i=1:length(eps_list)
                            if Crit(eps_list(i))>0
                                dd=dd+Wlist(eps_list(i))*volume(eps_list(i));
                            end
                        end
                        
                        D_energy = D_energy + ...
                            (status(e,1)-status(e,2))*dd;
                        Wlist(e)=Wlist(e)*(1-status(e,1));
                    else
                    	T=zeros(3);
                    end
                end
