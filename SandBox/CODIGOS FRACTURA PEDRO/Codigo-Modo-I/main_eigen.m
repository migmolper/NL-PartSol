...
Mat(1)=31e3;                            % E (MPa)
Mat(2)=0.2;                             % nu
Mat(3)=2368e-12;                        % rho (Tn/mm3)
Mat(4)=Mat(1)/2/(1+Mat(2));             % G (MPa)
Mat(5)=2*Mat(4)*Mat(2)/(1-2*Mat(2));    % Lambda (MPa)
M=Mat(1)*(1-Mat(2))/((1+Mat(2))*(1-2*Mat(2)));
Mat(6)=sqrt(M/Mat(3));                  % cp (mm/s)
%Fracture
...
tf       = 7.0e-4;         % Time of application of the load, s
VMAX     = -2640;          % mm/s
...
for nodo=1:nodes
    ...
    if  x_0(nodo,2)==H1 && x_0(nodo,1)<(M1+wi/2) && x_0(nodo,1)>(M1-wi/2)
        dad(nodo*df)=tf*VMAX;
        boundary(nodo*df)=1;
    end
end
...
Time_final= 3.0e-4;   %seconds
time_step = 2.5e-7;   %seconds
...
SAVE_I=2;     % Save info each XX steps
SAVE_F=500;     % Save the file each XX steps
